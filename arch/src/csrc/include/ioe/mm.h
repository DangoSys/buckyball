...
class mm_t {
public:
  mm_t(size_t mem_bs, size_t mem_sz, size_t word_sz, size_t line_sz,
       backing_data_t &dat)
      : data(dat.data), mem_base(mem_bs), mem_size(mem_sz), word_size(word_sz),
        line_size(line_sz) {
...
public:
  mm_magic_t(size_t mem_base, size_t mem_sz, size_t word_sz, size_t line_sz,
             backing_data_t &dat)
  ...
  virtual void *r_data() {
    return r_valid() ? &rresp.front().data[0] : (void *)data;
  }
