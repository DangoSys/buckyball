...
extern "C" void dpi_mtrace(unsigned char is_write, // 1 = write, 0 = read
  ...
  if (mtrace_fp) {
    if (is_write) {
      fprintf(mtrace_fp,
              "[MTRACE] WRITE ch=%u vbank=%u group=%u addr=0x%08x "
              "data=0x%016llx%016llx\n",
              channel, vbank_id, group_id, addr, data_hi, data_lo);
    } else {
      fprintf(mtrace_fp, "[MTRACE] READ  ch=%u vbank=%u group=%u addr=0x%08x\n",
              channel, vbank_id, group_id, addr);
    }
    fflush(mtrace_fp);
  }
}
