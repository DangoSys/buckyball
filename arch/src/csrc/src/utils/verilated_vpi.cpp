...
#define VL_VPI_INTERNAL_                                                       \
  VerilatedVpiImp::error_info()->setMessage(vpiInternal)->setMessage
#define VL_VPI_SYSTEM_                                                         \
  VerilatedVpiImp::error_info()->setMessage(vpiSystem)->setMessage
#define VL_VPI_ERROR_                                                          \
  VerilatedVpiImp::error_info()->setMessage(vpiError)->setMessage
#define VL_VPI_WARNING_                                                        \
  VerilatedVpiImp::error_info()->setMessage(vpiWarning)->setMessage
#define VL_VPI_NOTICE_                                                         \
  VerilatedVpiImp::error_info()->setMessage(vpiNotice)->setMessage
#define VL_VPI_ERROR_RESET_ VerilatedVpiImp::error_info()->resetError

// Not supported yet
#define VL_VPI_UNIMP_()                                                        \
  (VL_VPI_ERROR_(__FILE__, __LINE__,                                           \
                 Verilated::catName("Unsupported VPI function: ", __func__)))
...
public:
  ...
  static void *operator new(size_t size) VL_MT_SAFE {
    ...
    // To simplify our free list, we use a size large enough for all derived
    // types We reserve word zero for the next pointer, as that's safer in case
    // a dangling reference to the original remains around.
  ...
  static void operator delete(void *obj, size_t /*size*/) VL_MT_SAFE {
    ...
    if (VL_UNLIKELY(*(reinterpret_cast<uint32_t *>(oldp)) != activeMagic())) {
      VL_FATAL_MT(__FILE__, __LINE__, "",
                  "vpi_release_handle() called on same object twice, or on "
                  "non-Verilator "
                  "VPI object");
    }
...
public:
  ...
  VerilatedVpioReasonCb(uint64_t id, QData time, PLI_INT32 reason)
      : m_id{id}, m_time{time}, m_reason{reason} {}
  ~VerilatedVpioReasonCb() override = default;
  static VerilatedVpioReasonCb *castp(vpiHandle h) {
    return dynamic_cast<VerilatedVpioReasonCb *>(
        reinterpret_cast<VerilatedVpioReasonCb *>(h));
  }
...
public:
  explicit VerilatedVpioConst(int32_t num) : m_num{num} {}
  ~VerilatedVpioConst() override = default;
  static VerilatedVpioConst *castp(vpiHandle h) {
    return dynamic_cast<VerilatedVpioConst *>(
        reinterpret_cast<VerilatedVpio *>(h));
  }
...
public:
  ...
  static VerilatedVpioVarBase *castp(vpiHandle h) {
    return dynamic_cast<VerilatedVpioVarBase *>(
        reinterpret_cast<VerilatedVpio *>(h));
  }
...
public:
  ...
  static VerilatedVpioParam *castp(vpiHandle h) {
    return dynamic_cast<VerilatedVpioParam *>(
        reinterpret_cast<VerilatedVpio *>(h));
  }
...
public:
  ...
  static VerilatedVpioRange *castp(vpiHandle h) {
    return dynamic_cast<VerilatedVpioRange *>(
        reinterpret_cast<VerilatedVpio *>(h));
  }
...
public:
  ...
  static VerilatedVpioRangeIter *castp(vpiHandle h) {
    return dynamic_cast<VerilatedVpioRangeIter *>(
        reinterpret_cast<VerilatedVpio *>(h));
  }
...
public:
  explicit VerilatedVpioScope(const VerilatedScope *scopep) : m_scopep{scopep} {
...
public:
  ...
  ~VerilatedVpioVar() override {
    if (m_prevDatap)
      VL_DO_CLEAR(delete[] m_prevDatap, m_prevDatap = nullptr);
  }
  static VerilatedVpioVar *castp(vpiHandle h) {
    return dynamic_cast<VerilatedVpioVar *>(
        reinterpret_cast<VerilatedVpio *>(h));
  }
...
public:
  VerilatedVpioMemoryWord(const VerilatedVar *varp,
                          const VerilatedScope *scopep, int32_t index,
                          int offset)
  ...
  static VerilatedVpioMemoryWord *castp(vpiHandle h) {
    return dynamic_cast<VerilatedVpioMemoryWord *>(
        reinterpret_cast<VerilatedVpio *>(h));
  }
...
public:
  explicit VerilatedVpioVarIter(const VerilatedVpioScope *vop,
      ...
      : m_scopep{vop->scopep()}, m_onlyParams{onlyParams} {
    if (VL_UNLIKELY(vop->toplevel()))
      // This is a toplevel, so get TOP scope to search for ports during
      // vpi_scan.
  ...
  static VerilatedVpioVarIter *castp(vpiHandle h) {
    return dynamic_cast<VerilatedVpioVarIter *>(
        reinterpret_cast<VerilatedVpio *>(h));
  }
  uint32_t type() const override { return vpiIterator; }
  vpiHandle dovpi_scan() override {
    ...
    while (true) {
      ...
      if (m_onlyParams && !m_it->second.isParam())
        continue;
      if (VL_UNLIKELY(m_topscopep)) {
        if (const VerilatedVar *topvarp =
                m_topscopep->varFind(m_it->second.name())) {
          if (topvarp->isParam()) {
            return ((new VerilatedVpioParam{topvarp, m_topscopep})
                        ->castVpiHandle());
          } else {
            return (
                (new VerilatedVpioVar{topvarp, m_topscopep})->castVpiHandle());
          }
        }
      }
      if (m_it->second.isParam()) {
        return ((new VerilatedVpioParam{&(m_it->second), m_scopep})
                    ->castVpiHandle());
      } else {
        return (
            (new VerilatedVpioVar{&(m_it->second), m_scopep})->castVpiHandle());
      }
    }
  }
};
...
public:
  explicit VerilatedVpioModule(const VerilatedScope *modulep)
    ...
    if (std::strncmp(m_fullname, "TOP.", 4) == 0)
      m_fullname += 4;
    m_name = m_scopep->identifier();
  }
  static VerilatedVpioModule *castp(vpiHandle h) {
    return dynamic_cast<VerilatedVpioModule *>(
        reinterpret_cast<VerilatedVpio *>(h));
  }
...
public:
  explicit VerilatedVpioModuleIter(
      const std::vector<const VerilatedScope *> &vec)
      : m_vec{&vec} {
    m_it = m_vec->begin();
  }
  ~VerilatedVpioModuleIter() override = default;
  static VerilatedVpioModuleIter *castp(vpiHandle h) {
    return dynamic_cast<VerilatedVpioModuleIter *>(
        reinterpret_cast<VerilatedVpio *>(h));
  }
  uint32_t type() const override { return vpiIterator; }
  vpiHandle dovpi_scan() override {
...
public:
  explicit VerilatedVpioPackage(const VerilatedScope *modulep)
    ...
    if (std::strncmp(sfullname, "TOP.", 4) == 0)
      sfullname += 4;
  ...
  static VerilatedVpioPackage *castp(vpiHandle h) {
    return dynamic_cast<VerilatedVpioPackage *>(
        reinterpret_cast<VerilatedVpio *>(h));
  }
...
public:
  explicit VerilatedVpioInstanceIter(
      const std::vector<const VerilatedScope *> &vec)
      : m_vec{&vec} {
    m_it = m_vec->begin();
  }
  ...
  static VerilatedVpioInstanceIter *castp(vpiHandle h) {
    return dynamic_cast<VerilatedVpioInstanceIter *>(
        reinterpret_cast<VerilatedVpio *>(h));
  }
  uint32_t type() const override { return vpiIterator; }
  vpiHandle dovpi_scan() override {
...
class VerilatedVpiCbHolder final {
  ...
  uint64_t
      m_id; // Unique id/sequence number to find schedule's event, 0 = invalid
  s_cb_data m_cbData;
  s_vpi_value m_value;
  VerilatedVpioVar
      m_varo; // If a cbValueChange callback, the object we will return

public:
  // cppcheck-suppress uninitVar  // m_value
  VerilatedVpiCbHolder(uint64_t id, const s_cb_data *cbDatap,
                       const VerilatedVpioVar *varop)
      : m_id{id}, m_cbData{*cbDatap}, m_varo{varop} {
...
class VerilatedVpiImp final {
  ...
  using VpioFutureCbs =
      std::map<std::pair<QData, uint64_t>, VerilatedVpiCbHolder>;
...
public:
  ...
  static void cbCurrentAdd(uint64_t id, const s_cb_data *cb_data_p) {
    ...
    if (cb_data_p->reason == cbValueChange)
      varop = VerilatedVpioVar::castp(cb_data_p->obj);
  ...
  static void cbReasonRemove(uint64_t id, uint32_t reason, QData time) {
    // Id might no longer exist, if already removed due to call after event, or
    // teardown We do not remove it now as we may be iterating the list, instead
    // set to nullptr and will cleanup later Remove from cbCurrent queue
  ...
  static void moveFutureCbs() VL_MT_UNSAFE_ONE {
    // For any events past current time, move from cbFuture queue to cbCurrent
    // queue
    if (s().m_futureCbs.empty() && s().m_nextCbs.empty())
      return;
    ...
    for (auto it = s().m_futureCbs.begin(); //
      ...
      if (VL_UNLIKELY(!hor.invalid())) {
        VL_DEBUG_IF_PLI(
            VL_DBG_MSGF("- vpi: moveFutureCbs id=%" PRId64 "\n", hor.id()););
  ...
  static QData cbNextDeadline() {
    ...
    if (VL_LIKELY(it != s().m_futureCbs.cend()))
  ...
  static bool callCbs(const uint32_t reason) VL_MT_UNSAFE_ONE {
    ...
    assertOneCheck();
    moveFutureCbs();
    if (s().m_cbCurrentLists[reason].empty())
      return false;
    // Iterate on old list, making new list empty, to prevent looping over newly
    // added elements
    VpioCbList cbObjList;
    std::swap(s().m_cbCurrentLists[reason], cbObjList);
    bool called = false;
    for (VerilatedVpiCbHolder &ihor : cbObjList) {
      // cbReasonRemove sets to nullptr, so we know on removal the old end()
      // will still exist
      if (VL_LIKELY(!ihor.invalid())) { // Not deleted earlier
        VL_DEBUG_IF_PLI(
            VL_DBG_MSGF("- vpi: reason_callback reason=%d id=%" PRId64 "\n",
                        reason, ihor.id()););
  ...
  static bool callValueCbs() VL_MT_UNSAFE_ONE {
    ...
    bool called = false;
    std::unordered_set<VerilatedVpioVar *>
        update; // set of objects to update after callbacks
    if (cbObjList.empty())
      return called;
    const auto last =
        std::prev(cbObjList.end()); // prevent looping over newly added elements
    for (auto it = cbObjList.begin(); true;) {
      ...
      if (std::memcmp(prevDatap, newDatap, varop->entSize()) != 0) {
        VL_DEBUG_IF_PLI(VL_DBG_MSGF("- vpi: value_callback %" PRId64
                                    " %s v[0]=%d\n",
  ...
  static VerilatedVpiError *
  error_info() VL_MT_UNSAFE_ONE; // getter for vpi error info
};
...
class VerilatedVpiError final {
  ...
  void setError(PLI_BYTE8 *message, PLI_BYTE8 *code, PLI_BYTE8 *file,
                PLI_INT32 line) {
  ...
  void do_callbacks() {
    if (getError()->level >= vpiError &&
        Verilated::threadContextp()->fatalOnVpiError()) {
...
public:
  ...
  static void vpi_unsupported() {
    // Not supported yet
    const p_vpi_error_info error_info_p =
        VerilatedVpiImp::error_info()->getError();
    if (error_info_p) {
      VL_FATAL_MT(error_info_p->file, error_info_p->line, "",
                  error_info_p->message);
      return;
    }
    VL_FATAL_MT(__FILE__, __LINE__, "",
                "vpi_unsupported called without error info set");
  }
...
void VerilatedVpi::callTimedCbs() VL_MT_UNSAFE_ONE {
  VerilatedVpiImp::callCbs(cbAfterDelay);
}

bool VerilatedVpi::callValueCbs() VL_MT_UNSAFE_ONE {
  return VerilatedVpiImp::callValueCbs();
}
...
VerilatedVpiError *VerilatedVpiImp::error_info() VL_MT_UNSAFE_ONE {
  ...
  if (VL_UNLIKELY(!s().m_errorInfop))
    s().m_errorInfop = new VerilatedVpiError;
...
const char *
VerilatedVpiError::strFromVpiCallbackReason(PLI_INT32 vpiVal) VL_PURE {
  // clang-format off
    static const char* const names[] = {
        "*undefined*",
        "cbValueChange",
...
const char *
VerilatedVpiError::strFromVpiConstType(PLI_INT32 constType) VL_PURE {
  // clang-format off
    static const char* const names[] = {
        "*undefined*",
        "vpiDecConst",
...
vpiHandle vpi_register_cb(p_cb_data cb_data_p) {
  // Returns handle so user can remove the callback, user must
  // vpi_release_handle it Don't confuse with the callback-activated t_cb_data
  // object handle which is the object causing the callback rather than the
  // callback itself
  VerilatedVpiImp::assertOneCheck();
  VL_VPI_ERROR_RESET_();
  // cppcheck-suppress nullPointer
  if (VL_UNLIKELY(!cb_data_p)) {
    VL_VPI_WARNING_(__FILE__, __LINE__, "%s : callback data pointer is null",
                    __func__);
    return nullptr;
  }
  ...
  case cbAtEndOfSimTime:   // FALLTHRU // One-shot; time absolute; supported via
                           // vlt_main.cpp
  case cbAtStartOfSimTime: // FALLTHRU // One-shot; time absolute; supported via
                           // vlt_main.cpp
  case cbReadOnlySynch:    // FALLTHRU // One-shot; time relative; supported via
                           // vlt_main.cpp
  case cbReadWriteSynch: { // One-shot; time relative; supported via
                           // vlt_main.cpp
    ...
    if (cb_data_p->time) {
      if (abs) {
        ...
        abstime =
            time + VL_SET_QII(cb_data_p->time->high, cb_data_p->time->low);
      }
    }
    const uint64_t id = VerilatedVpiImp::nextCallbackId();
    VerilatedVpioReasonCb *const vop =
        new VerilatedVpioReasonCb{id, abstime, reason};
    if (abstime <= time) {
  ...
  case cbNextSimTime: { // One-shot; time always next; supported via
                        // vlt_main.cpp
    const QData time = VL_TIME_Q();
    const uint64_t id = VerilatedVpiImp::nextCallbackId();
  ...
  case cbEndOfSimulation:  // FALLTHRU // One-shot; time ignored; supported via
                           // vlt_main.cpp
  case cbEnterInteractive: // FALLTHRU // NOP, but need to return handle, so
                           // make object
  case cbExitInteractive: // FALLTHRU // NOP, but need to return handle, so make
                          // object
  case cbInteractiveScopeChange: // FALLTHRU // NOP, but need to return handle,
                                 // so make object
  case cbPLIError: // FALLTHRU // NOP, but need to return handle, so make object
  case cbStartOfSimulation: // FALLTHRU // One-shot; time ignored; supported via
                            // vlt_main.cpp
  ...
  default:
    VL_VPI_WARNING_(__FILE__, __LINE__, "%s: Unsupported callback type %s",
                    __func__,
                    VerilatedVpiError::strFromVpiCallbackReason(reason));
    return nullptr;
  }
...
void vpi_get_systf_info(vpiHandle /*object*/,
...
vpiHandle vpi_handle_by_name(PLI_BYTE8 *namep, vpiHandle scope) {
  VerilatedVpiImp::assertOneCheck();
  VL_VPI_ERROR_RESET_();
  if (VL_UNLIKELY(!namep))
    return nullptr;
  VL_DEBUG_IF_PLI(
      VL_DBG_MSGF("- vpi: vpi_handle_by_name %s %p\n", namep, scope););
  ...
  if (voScopep) {
    ...
    scopeAndName =
        std::string{voScopep->fullname()} + (scopeIsPackage ? "" : ".") + namep;
  ...
  }
  {
    ...
    while (i < scopeAndName.length()) {
      if (scopeAndName[i] == '\\') {
        while (i < scopeAndName.length() && scopeAndName[i] != ' ')
          ++i;
        ++i; // Proc ' ', it should always be there. Then grab '.' on next cycle
      } else {
        while (i < scopeAndName.length() &&
    ...
    if (prevpos == std::string::npos) {
      // scopename is a toplevel (no '.' separator), so search in our TOP ports
      // first.
...
vpiHandle vpi_handle_by_index(vpiHandle object, PLI_INT32 indx) {
  ...
  VL_DEBUG_IF_PLI(
      VL_DBG_MSGF("- vpi: vpi_handle_by_index %p %d\n", object, indx););
  VerilatedVpiImp::assertOneCheck();
  VL_VPI_ERROR_RESET_();
  // Memory words are not indexable
  const VerilatedVpioMemoryWord *const vop =
      VerilatedVpioMemoryWord::castp(object);
  if (VL_UNLIKELY(vop))
    return nullptr;
  const VerilatedVpioVar *const varop = VerilatedVpioVar::castp(object);
  if (VL_LIKELY(varop)) {
    if (varop->varp()->dims() < 2)
      return nullptr;
    if (VL_LIKELY(varop->varp()->unpacked().left() >=
                  varop->varp()->unpacked().right())) {
      if (VL_UNLIKELY(indx > varop->varp()->unpacked().left() ||
                      indx < varop->varp()->unpacked().right()))
        return nullptr;
      return (new VerilatedVpioMemoryWord{
                  varop->varp(), varop->scopep(), indx,
                  indx - varop->varp()->unpacked().right()})
          ->castVpiHandle();
    }
    if (VL_UNLIKELY(indx < varop->varp()->unpacked().left() ||
...
vpiHandle vpi_handle(PLI_INT32 type, vpiHandle object) {
  ...
  switch (type) {
  case vpiLeftRange: {
    if (const VerilatedVpioVarBase *const vop =
            VerilatedVpioVarBase::castp(object)) {
      if (VL_UNLIKELY(!vop->rangep()))
        return nullptr;
      return (new VerilatedVpioConst{vop->rangep()->left()})->castVpiHandle();
    } else if (const VerilatedVpioRange *const vop =
                   VerilatedVpioRange::castp(object)) {
      if (VL_UNLIKELY(!vop->rangep()))
        return nullptr;
      return (new VerilatedVpioConst{vop->rangep()->left()})->castVpiHandle();
    }
...
vpiHandle vpi_iterate(PLI_INT32 type, vpiHandle object) {
  ...
  case vpiMemoryWord: {
    ...
    return (new VerilatedVpioMemoryWordIter{object, vop->varp()})
        ->castVpiHandle();
  }
  case vpiRange: {
    const VerilatedVpioVar *const vop = VerilatedVpioVar::castp(object);
  ...
  case vpiModule: {
    ...
    const auto it =
        vlstd::as_const(map)->find(const_cast<VerilatedScope *>(modp));
    if (it == map->end())
      return nullptr;
...
PLI_INT32 vpi_get(PLI_INT32 property, vpiHandle object) {
  // Leave this in the header file - in many cases the compiler can constant
  // propagate "object"
  ...
  case vpiTimeUnit: {
    ...
    if (!vop)
      return Verilated::threadContextp()
          ->timeunit(); // Null asks for global, not unlikely
...
PLI_BYTE8 *vpi_get_str(PLI_INT32 property, vpiHandle object) {
  ...
  case vpiConstType: {
    ...
    VL_VPI_ERROR_RESET_();
    return const_cast<PLI_BYTE8 *>(
        VerilatedVpiError::strFromVpiConstType(constType));
  }
  default:
    VL_VPI_WARNING_(__FILE__, __LINE__,
                    "%s: Unsupported type %s, nothing will be returned",
...
// value processing
bool vl_check_format(const VerilatedVar *varp, const p_vpi_value valuep,
  ...
  VL_VPI_ERROR_(__FILE__, __LINE__, "%s: Unsupported format (%s) for %s",
...
void vl_get_value(const VerilatedVar *varp, void *varDatap, p_vpi_value valuep,
                  const char *fullname) {
  if (!vl_check_format(varp, valuep, fullname, true))
    return;
  // Maximum required size is for binary string, one byte per bit plus null
  // termination
  static thread_local char
      t_outStr[VL_VALUE_STRING_MAX_WORDS * VL_EDATASIZE + 1];
  ...
  // We used to presume vpiValue.format = vpiIntVal or if single bit
  // vpiScalarVal This may cause backward compatibility issues with older code.
  if (valuep->format == vpiVectorVal) {
    ...
    if (varp->vltype() == VLVT_UINT8) {
      ...
      if (VL_UNCOVERABLE(words >= VL_VALUE_STRING_MAX_WORDS)) {
        VL_FATAL_MT(__FILE__, __LINE__, "",
                    "vpi_get_value with more than VL_VALUE_STRING_MAX_WORDS; "
                    "increase and "
                    "recompile");
      }
    ...
    if (bits > t_outStrSz) {
      ...
      VL_VPI_WARNING_(__FILE__, __LINE__,
                      ...
                      " as buffer size (%d, VL_VALUE_STRING_MAX_WORDS=%d) is "
                      "less than required (%d)",
                      __func__,
                      VerilatedVpiError::strFromVpiVal(valuep->format),
                      fullname, t_outStrSz, VL_VALUE_STRING_MAX_WORDS, bits);
      bits = t_outStrSz;
    }
    for (i = 0; i < bits; ++i) {
    ...
    if (chars > t_outStrSz) {
      ...
      VL_VPI_WARNING_(__FILE__, __LINE__,
                      ...
                      __func__,
                      VerilatedVpiError::strFromVpiVal(valuep->format),
                      fullname, t_outStrSz, VL_VALUE_STRING_MAX_WORDS, chars);
      chars = t_outStrSz;
    }
    for (i = 0; i < chars; ++i) {
    ...
    if (varp->vltype() == VLVT_STRING) {
      ...
      for (i = 0; i < bytes; ++i) {
        ...
        // other simulators replace [leading?] zero chars with spaces, replicate
        // here.
        t_outStr[i] = val ? val : ' ';
      }
      t_outStr[i] = '\0';
  ...
  VL_VPI_ERROR_(__FILE__, __LINE__,
                "%s: Unsupported format (%s) as requested for %s", __func__,
...
void vpi_get_value(vpiHandle object, p_vpi_value valuep) {
  ...
  if (const VerilatedVpioVar *const vop = VerilatedVpioVar::castp(object)) {
    ...
    return;
  } else if (const VerilatedVpioParam *const vop =
                 VerilatedVpioParam::castp(object)) {
    vl_get_value(vop->varp(), vop->varDatap(), valuep, vop->fullname());
    return;
  } else if (const VerilatedVpioConst *const vop =
                 VerilatedVpioConst::castp(object)) {
  ...
  VL_VPI_ERROR_(__FILE__, __LINE__, "%s: Unsupported vpiHandle (%p)", __func__,
                object);
}

vpiHandle vpi_put_value(vpiHandle object, p_vpi_value valuep,
  ...
  if (const VerilatedVpioVar *const vop = VerilatedVpioVar::castp(object)) {
    VL_DEBUG_IF_PLI(
        VL_DBG_MSGF("- vpi:   vpi_put_value name=%s fmt=%d vali=%d\n",
                    vop->fullname(), valuep->format, valuep->value.integer);
    ...
    if (!vl_check_format(vop->varp(), valuep, vop->fullname(), false))
      return nullptr;
    if (valuep->format == vpiVectorVal) {
      if (VL_UNLIKELY(!valuep->value.vector))
        return nullptr;
      if (vop->varp()->vltype() == VLVT_UINT8) {
        ...
        *(reinterpret_cast<QData *>(vop->varDatap())) =
            VL_SET_QII(valuep->value.vector[1].aval & vop->mask(),
                       valuep->value.vector[0].aval);
        return object;
      ...
      for (int i = 0; i < bits; ++i) {
        const char set =
            (i < len) ? (valuep->value.str[len - i - 1] == '1') : 0;
      ...
      const int success =
          std::sscanf(valuep->value.str, "%30llu%15s", &val, remainder);
      if (success < 1) {
        VL_VPI_ERROR_(__FILE__, __LINE__,
      ...
      if (vop->varp()->vltype() == VLVT_STRING) {
        *(reinterpret_cast<std::string *>(vop->varDatap())) = valuep->value.str;
        ...
        for (int i = 0; i < bytes; ++i) {
          // prepend with 0 values before placing string the least significant
          // bytes
    ...
    return nullptr;
                 ...
                 VerilatedVpioParam::castp(object)) {
    VL_VPI_WARNING_(__FILE__, __LINE__,
                    "%s: Ignoring vpi_put_value to vpiParameter: %s", __func__,
                    vop->fullname());
    return nullptr;
  } else if (const VerilatedVpioConst *const vop =
                 VerilatedVpioConst::castp(object)) {
    VL_VPI_WARNING_(__FILE__, __LINE__,
                    "%s: Ignoring vpi_put_value to vpiConstant: %s", __func__,
                    vop->fullname());
    return nullptr;
  }
  VL_VPI_ERROR_(__FILE__, __LINE__, "%s: Unsupported vpiHandle (%p)", __func__,
...
void vpi_get_time(vpiHandle object, p_vpi_time time_p) {
  ...
  if (time_p->type == vpiSimTime) {
    ...
    if (const VerilatedVpioScope *const vop =
            VerilatedVpioScope::castp(object)) {
      const int scalePow10 = Verilated::threadContextp()->timeprecision() -
                             vop->scopep()->timeunit();
  ...
  VL_VPI_ERROR_(__FILE__, __LINE__, "%s: Unsupported type (%d)", __func__,
                time_p->type);
}
...
PLI_INT32 vpi_chk_error(p_vpi_error_info error_info_p) {
  ...
  VerilatedVpiImp::assertOneCheck();
  p_vpi_error_info const _error_info_p =
      VerilatedVpiImp::error_info()->getError();
...
PLI_INT32 vpi_get_vlog_info(p_vpi_vlog_info vlog_info_p) {
  // This is VL_MT_SAFE, but not marked as can't indicate it in the standardized
  // header file
  VerilatedVpiImp::assertOneCheck();
  VL_VPI_ERROR_RESET_();
...
PLI_INT32 vpi_get_data(PLI_INT32 /*id*/, PLI_BYTE8 * /*dataLoc*/,
                       PLI_INT32 /*numOfBytes*/) {
  VL_VPI_UNIMP_();
  return 0;
}
PLI_INT32 vpi_put_data(PLI_INT32 /*id*/, PLI_BYTE8 * /*dataLoc*/,
                       PLI_INT32 /*numOfBytes*/) {
  VL_VPI_UNIMP_();
  return 0;
}
...
PLI_INT32 vpi_control(PLI_INT32 operation, ...) {
  ...
  default: {
    VL_VPI_WARNING_(__FILE__, __LINE__, "%s: Unsupported type %s, ignoring",
                    __func__, VerilatedVpiError::strFromVpiProp(operation));
    return 0;
  }
  }
}
