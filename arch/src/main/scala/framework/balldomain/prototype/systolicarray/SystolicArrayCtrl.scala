...
class SystolicArrayCtrl(val b: GlobalConfig) extends Module {
  ...
  when(io.cmdReq.fire) {
    ...
    configReg      := io.cmdReq.bits.cmd.special
  ...
  val allCtrlReady = io.ctrl_ld_o.ready && io.ctrl_ex_o.ready && io.ctrl_st_o.ready
  ...
  when(state === busy && !has_send) {
    ...
    io.ctrl_st_o.bits.config       := configReg
