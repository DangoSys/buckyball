...
class PipelinedRelu(val b: GlobalConfig) extends Module {
  ...
  val io = IO(new Bundle {
    ...
    val bankRead =
      Vec(b.memDomain.bankNum, Flipped(new SramReadIO(b)))

    val bankWrite = Vec(
      b.memDomain.bankNum,
      Flipped(new SramWriteIO(b))
    )
  ...
  for (i <- 0 until b.memDomain.bankNum) {
    ...
    io.bankWrite(i).req.bits.mask := VecInit(Seq.fill(b.memDomain.bankMaskLen)(0.U(1.W)))
  }
