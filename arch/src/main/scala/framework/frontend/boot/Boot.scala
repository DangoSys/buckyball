...
class BootRom(val b: GlobalConfig) extends Module {
  ...
  val bootRs1Data         = VecInit(bootRecords.map(r => r.rs1.U(b.cpu.xLen.W)))
  val bootRs2Data         = VecInit(bootRecords.map(r => r.rs2.U(b.cpu.xLen.W)))
  ...
  val current = Wire(new RoCCCommandBB(b.cpu.xLen))
  current         := 0.U.asTypeOf(new RoCCCommandBB(b.cpu.xLen))
