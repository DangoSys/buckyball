...
import framework.system.configloader.ChipBundleLoader

/**
 * Build an N-BBTile chipyard subsystem from a ChipBundle `.pb` file generated by
 * `chip_bundle.py` / Bazel `:bundle`.
 */
class WithBuckyballTiles(
  bundlePath:     String,
  withBuckyball:  Boolean = true,
  hiddenHartBase: Option[Int] = None)
    extends Config(WithBuckyballTiles.assemble(bundlePath, withBuckyball, hiddenHartBase))

object WithBuckyballTiles {

  def assemble(bundlePath: String, withBuckyball: Boolean, hiddenHartBase: Option[Int]): Parameters = {
    if (!bundlePath.endsWith(".pb")) {
      throw new RuntimeException(s"WithBuckyballTiles expects a ChipBundle .pb path, got: $bundlePath")
    }
    val topology = ChipBundleLoader.load(bundlePath)
