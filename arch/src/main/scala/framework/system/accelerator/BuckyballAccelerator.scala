package framework.system.accelerator
...
import framework.system.accelerator.memdomain.MemDomain
...
import framework.system.sm.rocket.{RoCCCommandBB, RoCCResponseBB}
 ...
 * AXI4 bundles are exposed for the outer Core layer to bridge to TileLink.
 *
...
class BuckyballAccelerator(val b: GlobalConfig) extends Module {
  ...
  val io = IO(new Bundle {
    ...
    // AXI4 DMA bundles (bridged to TileLink in the outer Core layer)
  ...
  // --- AXI4 DMA ---
  io.axi_reader <> memDomain.io.axi_reader
  io.axi_writer <> memDomain.io.axi_writer
