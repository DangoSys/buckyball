...
package framework.system.sm.rocket
...
import framework.system.sm.rocket.RVVRoCCDecode

case class RocketCoreParams(
  hasBuckyball:           Boolean,
  hasBeu:                 Boolean,
  nTotalRoCCCSRs:         Int = 0,
  dcacheFlushOnFenceI:    Boolean = false,
  dcacheCanSupportCFlush: Boolean = false,
  roccCSRs:               Seq[freechips.rocketchip.tile.CustomCSR] = Nil)
...
class RocketCore(rcParams: RocketCoreParams)(implicit p: Parameters)
    extends CoreModule()(p)
    with HasRocketCoreParameters
    with HasRocketCoreIO {
  def nTotalRoCCCSRs = rcParams.nTotalRoCCCSRs
  def hasBeu         = rcParams.hasBeu

  // Override usingRoCC: when hasBuckyball is enabled the core must still
  // treat custom instructions as RoCC commands.
  override val usingRoCC = rcParams.hasBuckyball
  ...
  class RocketImpl { // entering gated-clock domain
        ...
        Seq(
          ...
          ++ (if (!usingFPU) Seq()
                ...
                Seq(
                  ...
                  (
                    "fp other",
                    () => id_ctrl.fp && !(io.fpu.dec.ldst || io.fpu.dec.fma || io.fpu.dec.div || io.fpu.dec.sqrt)
                  )
                ))
      ),
      new EventSet(
        ...
        Seq(
          (
            "load-use interlock",
            () => id_ex_hazard && ex_ctrl.mem || id_mem_hazard && mem_ctrl.mem || id_wb_hazard && wb_ctrl.mem
          ),
          ("long-latency interlock", () => id_sboard_hazard),
          (
            "csr interlock",
            () =>
              id_ex_hazard && ex_ctrl.csr =/= CSR.N || id_mem_hazard && mem_ctrl.csr =/= CSR.N || id_wb_hazard && wb_ctrl.csr =/= CSR.N
          ),
          ...
          ++ (if (!usingMulDiv) Seq()
                ...
                Seq(
                  (
                    "mul/div interlock",
                    () =>
                      id_ex_hazard && (ex_ctrl.mul || ex_ctrl.div) || id_mem_hazard && (mem_ctrl.mul || mem_ctrl.div) || id_wb_hazard && wb_ctrl.div
                  )
                ))
          ++ (if (!usingFPU) Seq()
              else
                Seq(
                  (
                    "fp interlock",
                    () =>
                      id_ex_hazard && ex_ctrl.fp || id_mem_hazard && mem_ctrl.fp || id_wb_hazard && wb_ctrl.fp || id_ctrl.fp && id_stall_fpu
                  )
                ))
      ),
      new EventSet(
    ...
    val usingRVVRoCC = rcParams.hasBuckyball
    ...
    val decode_table = {
      (if (usingMulDiv) new MDecode(pipelinedMul) +: (xLen > 32).option(new M64Decode(pipelinedMul)).toSeq else Nil) ++:
        ...
        (if (minFLen == 16)
           new HDecode +: (xLen > 32).option(new H64Decode).toSeq ++: (fLen >= 64).option(new HDDecode).toSeq
         else Nil) ++:
        ...
        Seq(new FenceIDecode(rcParams.dcacheFlushOnFenceI)) ++:
        coreParams.haveCFlush.option(new CFlushDecode(rcParams.dcacheCanSupportCFlush)) ++:
    ...
    val id_set_vconfig =
      Seq(Instructions.VSETVLI, Instructions.VSETIVLI, Instructions.VSETVL).map(_ === id_inst(0)).orR && usingVector.B
    ...
    val id_illegal_insn = !id_ctrl.legal ||
      ...
      (id_ctrl.vec) && (csr.io.decode(0).vector_illegal || csr.io.vector.map(_.vconfig.vtype.vill).getOrElse(
        false.B
      )) ||
    ...
    val id_csr_rocc_write =
      rcParams.roccCSRs.map(_.id.U === id_inst(0)(31, 20)).foldLeft(false.B)(_ || _) && id_csr_en && !id_csr_ren
    ...
    val ex_op2_oh =
      UIntToOH(Mux(ex_ctrl.sel_alu2(0), (ex_reg_inst >> 20).asUInt, ex_rs(1))(log2Ceil(xLen) - 1, 0)).asSInt

    val ex_op2 = MuxLookup(ex_ctrl.sel_alu2, 0.S)(Seq(
      A2_RS2  -> ex_rs(1).asSInt,
      A2_IMM  -> ex_imm,
      A2_SIZE -> Mux(ex_reg_rvc, 2.S, 4.S)
    ) ++ (if (coreParams.useZbs)
            Seq(
              A2_RS2OH -> ex_op2_oh,
              A2_IMMOH -> ex_op2_oh
            )
    ...
    when(!ctrl_killd) {
      ...
      if (rcParams.dcacheFlushOnFenceI) {
    ...
    val ex_sfence          =
      usingVM.B && ex_ctrl.mem && (ex_ctrl.mem_cmd === M_SFENCE || ex_ctrl.mem_cmd === M_HFENCEV || ex_ctrl.mem_cmd === M_HFENCEG)
    ...
    val mem_br_target = mem_reg_pc.asSInt +
      Mux(
        mem_ctrl.branch && mem_br_taken,
        ImmGen(IMM_SB, mem_reg_inst),
        Mux(mem_ctrl.jal, ImmGen(IMM_UJ, mem_reg_inst), Mux(mem_reg_rvc, 2.S, 4.S))
      )

    val mem_npc = (Mux(
      mem_ctrl.jalr || mem_reg_sfence,
    ...
    val rf_wdata = Mux(
      ...
      Mux(
        ...
        Mux(
          wb_ctrl.csr =/= CSR.N,
          csr.io.rw.rdata,
          Mux(wb_ctrl.mul, mul.map(_.io.resp.bits.data).getOrElse(wb_reg_wdata), wb_reg_wdata)
        )
      )
    )
    ...
    csr.io.fcsr_flags.bits  := (io.fpu.fcsr_flags.bits & Fill(5, io.fpu.fcsr_flags.valid)) | (vector_fcsr_flags & Fill(
      5,
      vector_fcsr_flags_valid
    ))
    ...
    val (htval, mhtinst_read_pseudo) = {
      ...
      // read pseudoinstruction if a guest-page fault is caused by an implicit memory access for VS-stage address
      // translation
    ...
    if (rocketParams.debugROB.isDefined) {
      ...
      if (sz < 1) { // use unsynthesizable ROB
        ...
        DebugROB.pushTrace(clock, reset, io.hartid, csr_trace_with_wdata, should_wb, has_wb, wb_addr)
    ...
    val hazard_targets = Seq(
      (id_ctrl.rxs1 && id_raddr1 =/= 0.U, id_raddr1),
    ...
    val ex_cannot_bypass  =
      ex_ctrl.csr =/= CSR.N || ex_ctrl.jalr || ex_ctrl.mem || ex_ctrl.mul || ex_ctrl.div || ex_ctrl.fp || ex_ctrl.rocc || ex_ctrl.vec
    ...
    val mem_cannot_bypass  =
      mem_ctrl.csr =/= CSR.N || mem_ctrl.mem && mem_mem_cmd_bh || mem_ctrl.mul || mem_ctrl.div || mem_ctrl.fp || mem_ctrl.rocc || mem_ctrl.vec
    ...
    io.imem.might_request    := {
      imem_might_request_reg := ex_pc_valid || mem_pc_valid || io.ptw.customCSRs.disableICacheClockGate || io.vector.map(
        _.trap_check_busy
      ).getOrElse(false.B)
    ...
    io.imem.btb_update.bits.cfiType    :=
      Mux(
        (mem_ctrl.jal || mem_ctrl.jalr) && mem_waddr(0),
        CFIType.call,
        Mux(
          mem_ctrl.jalr && (mem_reg_inst(19, 15) & regAddrMask.U) === BitPat("b00?01"),
          CFIType.ret,
          Mux(mem_ctrl.jal || mem_ctrl.jalr, CFIType.jump, CFIType.branch)
        )
      )
    ...
    io.dmem.s1_data.data := (if (fLen == 0) mem_reg_rs2
                             else Mux(mem_ctrl.fp, Fill(coreDataBits / fLen, io.fpu.store_data), mem_reg_rs2))
    ...
    if (enableCommitLog) {
      ...
      when(csr.io.trace(0).valid) {
        printf(
          "C%d: %d [%d] pc=[%x] W[r%d=%x][%d] R[r%d=%x] R[r%d=%x] inst=[%x] DASM(%x) wb_xcpt:%d\n",
          io.hartid,
