package framework.gendomain.common
...
class ShiftPackerReq(implicit p: Parameters) extends CoreBundle()(p) with HasVectorParams {
  val head  = UInt(log2Ceil(dLenB).W)
  val tail  = UInt(log2Ceil(dLenB).W)
  def count = Mux(tail === 0.U, dLenB.U, tail) - head
}

class ShiftPacker(implicit p: Parameters) extends CoreModule()(p) with HasVectorParams {
  val io = IO(new Bundle {
    val push = Flipped(Decoupled(new ShiftPackerReq))
    val push_data = Input(Vec(dLenB, UInt(8.W)))

    val pop = Flipped(Decoupled(new ShiftPackerReq))
    val pop_data = Output(Vec(dLenB, UInt(8.W)))
  })
  ...
  val count = RegInit(0.U(log2Ceil(dLenB).W))
  ...
  val push_shr     = wshr((Seq.fill(dLenB)(0.U(8.W)) ++ io.push_data), dLenB.U +& io.push.bits.head - count)
  val push_shr_pop = wshr((Seq.fill(dLenB)(0.U(8.W)) ++ io.push_data), dLenB.U +& io.push.bits.head +& io.pop.bits.count - count)
