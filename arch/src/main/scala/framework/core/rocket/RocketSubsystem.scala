...
import freechips.rocketchip.prci.{ClockCrossingType, NoResetCrossing, ResetCrossingType, SynchronousCrossing}
import freechips.rocketchip.tile.{RocketTile, RocketTileParams}
import freechips.rocketchip.util.HasCoreMonitorBundles

import framework.core.rocket._
...
class RocketSubsystem(implicit p: Parameters)
...
class RocketSubsystemModuleImp[+L <: RocketSubsystem](_outer: L)
