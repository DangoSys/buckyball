...
class RocketTileBB private (
  ...
  val dtim_adapter = rocketParams.dcache.flatMap { d =>
    d.scratch.map { s =>
      LazyModule(new ScratchpadSlavePort(
        AddressSet.misaligned(s, d.dataScratchpadBytes),
        lazyCoreParamsView.coreDataBytes,
        rocketParams.core.useAtomics && !rocketParams.core.useAtomicsOnlyForIO
      ))
    }
  }
  ...
  nDCachePorts += 1 /*core */ + (dtim_adapter.isDefined).toInt + rocketParams.core.vector.map(
    _.useDCache.toInt
  ).getOrElse(0)

  val dtimProperty = dtim_adapter.map(d =>
    Map(
      "sifive,dtim" -> d.device.asProperty
    )
  ).getOrElse(Nil)

  val itimProperty = frontend.icache.itimProperty.toSeq.flatMap(p => Map("sifive,itim" -> p))

  val beuProperty = bus_error_unit.map(d =>
    Map(
      "sifive,buserror" -> d.device.asProperty
    )
  ).getOrElse(Nil)
