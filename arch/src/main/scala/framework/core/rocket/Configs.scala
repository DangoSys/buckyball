...
import freechips.rocketchip.prci.{AsynchronousCrossing, ClockCrossingType, RationalCrossing, SynchronousCrossing}
import freechips.rocketchip.subsystem.{
  CBUS,
  CCBUS,
  CacheBlockBytes,
  CloneTileAttachParams,
  ClustersLocated,
...
import freechips.rocketchip.tile.{FPUParams, RocketTileBoundaryBufferParams, RocketTileParams}
import scala.reflect.ClassTag

import framework.core.rocket.{RocketCrossingParamsBB, RocketTileParamsBB}
...
class WithNBuckyballCores(
  ...
  def this(n: Int, location: HierarchicalLocation = InSubsystem) = this(
    n,
    location,
    RocketCrossingParams(
...
class RocketTileAttachConfig(f: RocketTileAttachParams => RocketTileAttachParams)
    extends TileAttachConfig[RocketTileAttachParams](f)

class RocketTileConfig(f: RocketTileParams => RocketTileParams)
    extends RocketTileAttachConfig(tp =>
      tp.copy(
...
class RocketCoreConfig(f: RocketCoreParams => RocketCoreParams)
    extends RocketTileConfig(tp =>
      tp.copy(
        core = f(tp.core)
      )
    )

class RocketICacheConfig(f: ICacheParams => ICacheParams)
    extends RocketTileConfig(tp =>
      tp.copy(
...
class RocketDCacheConfig(f: DCacheParams => DCacheParams)
    extends RocketTileConfig(tp =>
      tp.copy(
...
class WithL1ICacheECC(dataECC: String, tagECC: String)
    extends RocketICacheConfig(_.copy(dataECC = Some(dataECC), tagECC = Some(tagECC)))
...
class WithL1DCacheECC(dataECC: String, tagECC: String)
    extends RocketDCacheConfig(_.copy(dataECC = Some(dataECC), tagECC = Some(tagECC)))
...
class WithBoundaryBuffers(buffers: Option[RocketTileBoundaryBufferParams] = Some(RocketTileBoundaryBufferParams(true)))
    extends RocketTileConfig(_.copy(boundaryBuffers = buffers))

class WithRV32
    extends RocketCoreConfig(c =>
      c.copy(
        xLen = 32,
        pgLevels = 2, // sv32
...
class WithDebugROB(enable: Boolean = true, size: Int = 0)
    extends RocketCoreConfig(_.copy(debugROB = Option.when(enable)(DebugROBParams(size))))
...
class WithCustomFastMulDiv(
  mUnroll:              Int = 8,
  mEarlyOut:            Boolean = true,
  dUnroll:              Int = 1,
  dEarlyOut:            Boolean = true,
  dEarlyOutGranularity: Int = 1)
    extends RocketCoreConfig(_.copy(mulDiv =
      Some(
        MulDivParams(
          mulUnroll = mUnroll,
          mulEarlyOut = mEarlyOut,
          divUnroll = dUnroll,
          divEarlyOut = dEarlyOut,
          divEarlyOutGranularity = dEarlyOutGranularity
        )
      )
    ))
...
// Tile CDC configs
class WithCDC(crossingType: ClockCrossingType = SynchronousCrossing())
    extends RocketCrossingConfig(_.copy(crossingType = crossingType))
