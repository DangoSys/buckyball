  ...
  for (i <- 0 until numChannels) {
    ...
    when(entry.valid) {
      ...
      when(entry.isRead) {
        ...
        io.mem_req(i).read.req.valid := RegNext(ballRead.req.valid)
        io.mem_req(i).read.req.bits  := RegNext(ballRead.req.bits)
      }.otherwise {
        io.mem_req(i).write <> ballWrite
        io.mem_req(i).bank_id         := wbank_id
        io.mem_req(i).write.req.valid := RegNext(ballWrite.req.valid)
        io.mem_req(i).write.req.bits  := RegNext(ballWrite.req.bits)
      }
    }
  }
