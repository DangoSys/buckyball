...
class Pipeline[T <: Data](gen: T, latency: Int)(comb: Seq[T => T] = Seq.fill(latency + 1)((x: T) => x)) extends Module {
  ...
  if (latency == 0) {
    ...
    (stalling.init, stalling.tail, valids.init).zipped.foreach {
      case (s1, s2, v1) =>
        s1 := v1 && s2
    }
    ...
    (valids.init, stalling.tail).zipped.foreach {
      case (v1, s2) =>
        when(!s2) {
          v1 := false.B
        }
    ...
    (valids.tail, valids.init).zipped.foreach {
      case (v2, v1) =>
        when(v1) {
          v2 := true.B
        }
    ...
    ((stages.tail zip stages.init) zip (stalling.tail zip comb.tail.init)).foreach {
      case ((st2, st1), (s2, c1)) =>
        when(!s2) {
          st2 := c1(st1)
        }
