package prototype.vector.configs
...
object VecConfig {
  implicit def rw: upickle.default.ReadWriter[VecConfig] = upickle.default.macroRW

  def fromBallDomain(ballParam: BallDomainParam): VecConfig = {
    VecConfig(
      ballParam = ballParam
    )
  }
   ...
   * Load from JSON file
   */
  def fromJson(path: String): VecConfig = {
    val jsonStr = scala.io.Source.fromFile(path).mkString
    upickle.default.read[VecConfig](jsonStr)
  }

  /**
   * Save to JSON file
   */
  def toJson(config: VecConfig, path: String): Unit = {
