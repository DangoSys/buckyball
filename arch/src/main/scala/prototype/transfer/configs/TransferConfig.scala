package prototype.transfer.configs
...
object TransferConfig {
  implicit def rw: upickle.default.ReadWriter[TransferConfig] = upickle.default.macroRW

  def fromBallDomain(ballParam: BallDomainParam): TransferConfig = {
    TransferConfig(
      ballParam = ballParam
    )
  }
   ...
   * Load from JSON file
   */
  def fromJson(path: String): TransferConfig = {
    val jsonStr = scala.io.Source.fromFile(path).mkString
    upickle.default.read[TransferConfig](jsonStr)
  }

  /**
   * Save to JSON file
   */
  def toJson(config: TransferConfig, path: String): Unit = {
