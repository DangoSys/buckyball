...
class GeluCtrlUnit(implicit b: CustomBuckyBallConfig, p: Parameters) extends Module {
  val io = IO(new Bundle {
    val cmdReq = Flipped(Decoupled(new BallRsIssue))
    val cmdResp_o = Decoupled(new BallRsComplete)

    val ctrl_ld_o = Decoupled(new CtrlLdReq)
    val ctrl_st_o = Decoupled(new CtrlStReq)
    val ctrl_ex_o = Decoupled(new CtrlExReq)
  ...
  when(state === busy && !has_send) {
    ...
    io.ctrl_ex_o.bits.is_acc         := is_acc
    ...
    // 只有当所有控制信号都被接收时才标记为已发送
    when(io.ctrl_ld_o.ready && io.ctrl_ex_o.ready && io.ctrl_st_o.ready) {
      has_send := true.B
    }
  }.otherwise {
    ...
    io.ctrl_ex_o.bits.is_acc         := false.B
