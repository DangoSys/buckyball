package prototype.relu.configs
...
object ReluConfig {
  implicit def rw: upickle.default.ReadWriter[ReluConfig] = upickle.default.macroRW

  def fromBallDomain(ballParam: BallDomainParam): ReluConfig = {
    ReluConfig(
      ballParam = ballParam
    )
  }
   ...
   * Load from JSON file
   */
  def fromJson(path: String): ReluConfig = {
    val jsonStr = scala.io.Source.fromFile(path).mkString
    upickle.default.read[ReluConfig](jsonStr)
  }

  /**
   * Save to JSON file
   */
  def toJson(config: ReluConfig, path: String): Unit = {
