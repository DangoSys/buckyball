...
/**
 * NNLut - Neural Network Look-Up Table unit
 * Simple implementation: read data from scratchpad, lookup in LUT, write back
 */
@instantiable
class NNLut(val parameter: NNLutConfig)(implicit p: Parameters) extends Module with SerializableModule[NNLutConfig] {
  ...
  // Simple LUT: 256 entries for 8-bit input
  // This is a simple example LUT, can be replaced with actual NN-LUT values
  val LUT_SIZE = 256

  val lut = VecInit(Seq.tabulate(LUT_SIZE) { i =>
    // Simple example: identity function with saturation
    // In real NN-LUT, this would contain pre-computed activation function values
    val input  = i.asSInt
    val output = Mux(input < -128.S, -128.S, Mux(input > 127.S, 127.S, input))
    output.asUInt
  })
  ...
  switch(state) {
    ...
    is(sRead) {
      ...
      // Receive response and perform LUT lookup
      ...
      when(io.bankRead(rbank_reg).resp.fire) {
        for (col <- 0 until InputNum) {
          ...
          val raw = io.bankRead(rbank_reg).resp.bits.data(hi, lo)
          // Perform LUT lookup
          // Convert signed value to unsigned index (0-255)
          // Take lower 8 bits as unsigned index for LUT
          val idx = raw(7, 0) // Use lower 8 bits as unsigned index
          regArray(respCounter)(col) := lut(idx)
        }
