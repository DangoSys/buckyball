package prototype.abft.configs
...
object ABFTConfig {
  implicit def rw: upickle.default.ReadWriter[ABFTConfig] = upickle.default.macroRW

  def fromBallDomain(ballParam: BallDomainParam): ABFTConfig = {
    ABFTConfig(
      ballParam = ballParam
    )
  }
   ...
   * Load from JSON file
   */
  def fromJson(path: String): ABFTConfig = {
    val jsonStr = scala.io.Source.fromFile(path).mkString
    upickle.default.read[ABFTConfig](jsonStr)
  }

  /**
   * Save to JSON file
   */
  def toJson(config: ABFTConfig, path: String): Unit = {
