...
class id_lu_req(implicit bbconfig: CustomBuckyBallConfig) extends Bundle {
...
class lu_ex_req(implicit bbconfig: CustomBuckyBallConfig) extends Bundle {
    val op1_bank      = UInt(log2Up(bbconfig.sp_banks).W)
    val op2_bank      = UInt(log2Up(bbconfig.sp_banks).W)
    val wr_bank       = UInt(log2Up(bbconfig.sp_banks).W)
    val wr_bank_addr  = UInt(log2Up(bbconfig.spad_bank_entries).W)
...
class ID_LU(implicit bbconfig: CustomBuckyBallConfig) extends Module{
...
class LU_EX(implicit bbconfig: CustomBuckyBallConfig) extends Module{
    val io = IO(new Bundle {
