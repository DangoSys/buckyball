...
import framework.memdomain.backend.banks.{SramReadResp, SramWriteIO}
import framework.balldomain.rs.{BallRsComplete, BallRsIssue}
import examples.toy.balldomain.BallDomainParam

@instantiable
class BBFP_EX(val parameter: BallDomainParam)(implicit p: Parameters)
    extends Module
  ...
  val io = IO(new Bundle {
    ...
    val sramReadResp = Vec(parameter.numBanks, Flipped(Decoupled(new SramReadResp(bankWidth))))
  ...
  for (i <- 0 until parameter.numBanks) {
    ...
    io.sramWrite(i).req.bits.mask := VecInit(Seq.fill(parameter.bankMaskLen)(false.B))
  }
  ...
  for (i <- 0 until parameter.numBanks) {
    ...
    io.accWrite(i).req.bits.mask := VecInit(Seq.fill(parameter.bankMaskLen)(true.B))
  }
    ...
    when(write_cycles < 16.U) {
      ...
      for (i <- 0 until parameter.numBanks / 2) {
        when((wr_bank_addr_base + write_cycles)(0) === 0.U) {
          ...
          io.accWrite(i + parameter.numBanks / 2).req.valid     := true.B
          io.accWrite(i + parameter.numBanks / 2).req.bits.addr := wr_bank_addr_base + (write_cycles >> 1.U)
          val idx = (write_cycles * 4.U + i.U)(5, 0) // 6 bits for 64 elements
          io.accWrite(i + parameter.numBanks / 2).req.bits.data := Cat(
            output_buffer(idx)(3),
            output_buffer(idx)(2),
            output_buffer(idx)(1),
            output_buffer(idx)(0)
          )
          io.accWrite(i + parameter.numBanks / 2).req.bits.mask := VecInit(Seq.fill(parameter.bankMaskLen)(true.B))
        }
      }
