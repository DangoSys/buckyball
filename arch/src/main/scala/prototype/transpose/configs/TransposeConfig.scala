package prototype.transpose.configs

import chisel3._
import chisel3.experimental.SerializableModuleParameter
import examples.toy.balldomain.BallDomainParam

object TransposeConfig {
  implicit def rw: upickle.default.ReadWriter[TransposeConfig] = upickle.default.macroRW

  def fromBallDomain(ballParam: BallDomainParam): TransposeConfig = {
    TransposeConfig(
      ballParam = ballParam
    )
  }
   ...
   * Load from JSON file
   */
  def fromJson(path: String): TransposeConfig = {
    val jsonStr = scala.io.Source.fromFile(path).mkString
    upickle.default.read[TransposeConfig](jsonStr)
  }

  /**
   * Save to JSON file
   */
  def toJson(config: TransposeConfig, path: String): Unit = {
