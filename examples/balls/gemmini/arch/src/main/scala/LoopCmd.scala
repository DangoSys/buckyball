...
class LoopWsConfig(val b: GlobalConfig) extends Bundle {
  ...
  val bank_a      = UInt(b.memDomain.vbankIdWidth.W)
  val bank_b      = UInt(b.memDomain.vbankIdWidth.W)
  val bank_c      = UInt(b.memDomain.vbankIdWidth.W)
  val low_d       = Bool()
}
...
class LoopConvWsConfig(val b: GlobalConfig) extends Bundle {
  ...
  val bank_input       = UInt(b.memDomain.vbankIdWidth.W)
  val bank_weight      = UInt(b.memDomain.vbankIdWidth.W)
  val bank_output      = UInt(b.memDomain.vbankIdWidth.W)
  val no_bias          = Bool()
}
