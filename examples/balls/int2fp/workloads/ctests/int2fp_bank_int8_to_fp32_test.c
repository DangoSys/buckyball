...
int main(void) {
  ...
  for (int i = 0; i < NELEM; ++i) {
    ...
    input_i8[i] = v;
    expected_bits[i] = int2fp_fp32((int32_t)v, SCALE_1_0);
    output_fp32[i] = 0.0f;
  }
  ...
  bb_mem_alloc(dst, BANK_LINES, 4);
  bb_mvin((uintptr_t)input_i8, src, BANK_LINES, 1);
  ...
  for (int i = 0; i < NELEM; ++i) {
    ...
    double ideal = (double)input_i8[i] * scale;
  ...
  printf("Int2Fp bank INT8-to-FP32 %s\n", passed ? "PASSED" : "FAILED");
  return passed ? 0 : 1;
}
