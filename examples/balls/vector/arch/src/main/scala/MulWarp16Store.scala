...
class MulWarp16Store(val b: GlobalConfig, lane: Int, accWidth: Int) extends Module {
  ...
  switch(state) {
    ...
    is(sReq) {
      for (g <- 0 until 4) {
        ...
        io.bankWrite(g).io.req.bits.mask := VecInit(Seq.fill(b.memDomain.bankMaskLen)(true.B))
        when(io.bankWrite(g).io.req.fire) {
          writeIssued(g) := true.B
        }
      }
      when((0 until 4).map(g => writeIssued(g) || io.bankWrite(g).io.req.fire).reduce(_ && _)) {
        writeAcked := clr4()
        state      := sResp
      }
    }
    ...
    is(sResp) {
      ...
      when((0 until 4).map(g => writeAcked(g) || io.bankWrite(g).io.resp.fire).reduce(_ && _)) {
