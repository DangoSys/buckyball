...
static void hw_vector_mul_warp16(elem_t *a, elem_t *b, result_t *c, int size) {
...
static int run_boot_init_test(void) {
  ...
  hw_vector_mul_warp16(input_matrix_a, input_matrix_b, output_matrix, DIM);

  if (compare_u32_matrices(output_matrix, expected_matrix, DIM, DIM)) {
    printf("vector_mul_warp16_test matmul PASSED\n");
    return 1;
  }

  printf("vector_mul_warp16_test matmul FAILED\n");
  return 0;
}
...
int main() {
  ...
  if (passed) {
    printf("vector_mul_warp16_test PASSED\n");
    return 0;
  }

  printf("vector_mul_warp16_test FAILED\n");
  return 1;
