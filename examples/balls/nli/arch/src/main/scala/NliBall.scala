  ...
  switch(state) {
    is(idle) {
      when(io.cmdReq.fire) {
        ...
        assert(cmd.iter > 0.U && cmd.iter <= b.memDomain.bankEntries.U,
          "NliBall iter must fit in one bank")
        assert(cmd.op1_col === 1.U && cmd.op2_col === 1.U && cmd.wr_col === 1.U,
          "NliBall requires col=1 input/table/output")
        assert(cmd.op1_bank =/= cmd.op2_bank && cmd.op1_bank =/= cmd.wr_bank &&
          cmd.op2_bank =/= cmd.wr_bank, "NliBall banks must be distinct")
