func.func private @check_result(memref<64x8xi32>) -> ()

func.func @main() -> i8 {
  %zeroI8 = arith.constant 0 : i8
  %zero = arith.constant 0 : index
  %one = arith.constant 1 : index
  %lines = arith.constant 64 : i64
  %stride = arith.constant 1 : i64
  %limit = arith.constant 64 : index
  %columns = arith.constant 8 : index
  %a = memref.alloc() alignment = 64 : memref<64x8xi32>
  %b = memref.alloc() alignment = 64 : memref<64x8xi32>
  %c = memref.alloc() alignment = 64 : memref<64x8xi32>
  ...
  scf.for %line = %zero to %limit step %one {
    scf.for %column = %zero to %columns step %one {
      ...
      memref.store %aValue, %a[%line, %column] : memref<64x8xi32>
      memref.store %bValue, %b[%line, %column] : memref<64x8xi32>
    }
  }
  ...
  func.call @check_result(%c) : (memref<64x8xi32>) -> ()
  memref.dealloc %a : memref<64x8xi32>
  memref.dealloc %b : memref<64x8xi32>
  memref.dealloc %c : memref<64x8xi32>
  return %zeroI8 : i8
}
