...
impl BallInstruction for MatAdd {
    fn exec(xs1: u64, xs2: u64, ctx: &mut ExecContext) -> u64 {
        ...
        let a = *ctx.config(a_bank);
        let b = *ctx.config(b_bank);
        let c = *ctx.config(c_bank);
        ...
        for group in 0..a.cols as usize {
            let a_pbank = pbank_group(ctx, a_bank, group as u64);
            let b_pbank = pbank_group(ctx, b_bank, group as u64);
            let c_pbank = pbank_group(ctx, c_bank, group as u64);
            for line in 0..iter {
                let offset = line * 16;
                for lane in 0..4 {
                    let byte = offset + lane * 4;
                    let a_value =
                        u32::from_le_bytes(ctx.banks[a_pbank][byte..byte + 4].try_into().unwrap());
                    let b_value =
                        u32::from_le_bytes(ctx.banks[b_pbank][byte..byte + 4].try_into().unwrap());
