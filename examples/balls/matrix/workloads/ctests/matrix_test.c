...
int test_ones() {
  ...
  */
  init_u8_random_matrix(input_matrix_a, DIM, DIM, 111);
  init_u8_random_matrix(input_matrix_b, DIM, DIM, 222);
  return run_test("Matrix Matmul", input_matrix_a, input_matrix_b, DIM);
}
...
int main() {
  ...
  if (passed) {
    printf("Matrix Matmul test PASSED\n");
    return 0;
  } else {
    printf("Matrix Matmul test FAILED\n");
    return 1;
  }
