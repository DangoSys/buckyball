...
_Static_assert(M >= BANK_LINES && K == MATRIX_TILE,
               "bank WS test must cover a full bank of A rows");
...
int main(void) {
  ...
  matrix_hw_mnk(pa, pb, pc, M, N, K, 1);
  matrix_unpack_c(pc, out, M, N);
  if (!compare_u32_matrices(out, exp_, M, N)) {
    printf("matrix_bank_ws_1024x16x16 FAILED\n");
    return 1;
  }
  printf("matrix_bank_ws_1024x16x16 PASSED\n");
  return 0;
}
