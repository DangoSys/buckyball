...
static void matrix_hw_banks(const elem_t *packed_a, const elem_t *packed_b,
  ...
  bb_matrix_mnk_mode(op1, op2, wr, m, n, k, BB_MATRIX_MODE_OS);
...
int main(void) {
  ...
  printf("matrix_os_seq_n6_n16 start\n");
  matrix_hw_banks(pa1, pb1, pc1, M1, N1, K1, 0, 5, 1);
  printf("matrix_os_seq_n6_n16 after_n6\n");
  matrix_hw_banks(pa2, pb2, pc2, M2, N2, K2, 0, 5, 1);
  printf("matrix_os_seq_n6_n16 DONE\n");
  return 0;
}
