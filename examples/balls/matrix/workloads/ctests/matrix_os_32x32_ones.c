...
static elem_t pb[64 * MATRIX_TILE] __attribute__((aligned(64)));
static result_t pc[64 * MATRIX_ACC_LANES] __attribute__((aligned(64)));
...
int main(void) {
  ...
  clear_u32_matrix(pc, 64, MATRIX_ACC_LANES);
  matrix_hw_mnk(pa, pb, pc, DIM, DIM, DIM, 0);
  matrix_unpack_c(pc, out, DIM, DIM);
  if (!compare_u32_matrices(out, exp_, DIM, DIM)) {
    printf("matrix_os_32x32_ones FAILED\n");
    return 1;
  }
  printf("matrix_os_32x32_ones PASSED\n");
  return 0;
}
