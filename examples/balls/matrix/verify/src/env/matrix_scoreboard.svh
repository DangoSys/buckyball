class matrix_scoreboard extends uvm_scoreboard;
  ...
  uvm_analysis_imp_stim #(bb_blink_cmd_item, matrix_scoreboard) stim_imp;
  uvm_analysis_imp_cmd #(bb_blink_cmd_item, matrix_scoreboard) cmd_imp;
  uvm_analysis_imp_read #(bb_blink_read_item, matrix_scoreboard) read_imp;
  uvm_analysis_imp_write #(bb_blink_write_item, matrix_scoreboard) write_imp;
  uvm_analysis_imp_resp #(bb_blink_resp_item, matrix_scoreboard) resp_imp;

  bb_blink_mem_model #(2, 4) mem_model;

  matrix_cmd_item stim_q[$];
  ...
  function void write_stim(bb_blink_cmd_item item);
    matrix_cmd_item mitem;
    matrix_cmd_item clone;
    int i;
    ...
    if (!$cast(mitem, item)) begin
      `uvm_fatal("SCB", "stim item is not matrix_cmd_item")
    end
    ...
    for (i = 0; i < clone.num_writes; i++) begin
      exp_group.push_back(matrix_case_write_group(i));
      exp_addr.push_back(matrix_case_write_addr(i));
      d_lo = matrix_case_write_data_lo(i);
      d_hi = matrix_case_write_data_hi(i);
      exp_data.push_back({d_hi, d_lo});
      exp_mask.push_back(matrix_case_write_mask(i) [15:0]);
  ...
  function void write_cmd(bb_blink_cmd_item item);
    matrix_cmd_item stim;
    stim = current_stim("CMD");
    check_cmd(item, stim);
    cmd_count++;
  endfunction

  function void check_cmd(bb_blink_cmd_item got, matrix_cmd_item exp);
    if (got.bid !== exp.bid)
  ...
  function matrix_cmd_item current_stim(string tag);
    if (stim_q.size() == 0) begin
