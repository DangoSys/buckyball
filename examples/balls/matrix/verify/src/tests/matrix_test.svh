class matrix_ball_test extends uvm_test;
  `uvm_component_utils(matrix_ball_test)
  ...
  task run_case(int unsigned idx, string label, int unsigned bid);
    ...
    while (!env.scb.done()) begin
      ...
      if (cycles > MATRIX_TIMEOUT_CYCLES) begin
        `uvm_fatal(
            "TIMEOUT",
            $sformatf("MatrixBall %s (idx=%0d) timeout: cmds=%0d r0=%0d r1=%0d wr=%0d/%0d resp=%0d",
    ...
    `uvm_info("MATRIX", $sformatf("MatrixBall %s (idx=%0d) passed in %0d cycles", label, idx,
                                  cycles), UVM_LOW)
  endtask
