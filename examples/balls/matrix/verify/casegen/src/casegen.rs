...
pub fn gen_case(seed: u32, index: u32, bid: u32) -> MatrixCase {
    match index {
        0 => directed_4x4(bid, 0),
        1 => directed_4x4(bid, 1),
        2 => directed_16x16_os(bid),
...
fn build_case(
    ...
    let rs2 = model::encode_rs2(m as u32, n as u32, k as u32, mode);
...
fn directed_4x4(bid: u32, mode: u32) -> MatrixCase {
    let m = 4usize;
    let n = 4usize;
    let k = 4usize;
    ...
    build_case(bid, m, n, k, mode, 0, 1, 2, 3, &a, &b)
}

fn directed_16x16_os(bid: u32) -> MatrixCase {
    let m = 16usize;
    let n = 16usize;
    let k = 16usize;
    ...
    build_case(bid, m, n, k, 0, 0, 1, 2, 7, &a, &b)
}

fn random_case(seed: u32, index: u32, bid: u32) -> MatrixCase {
    let mut rng = Rng::new(seed, index);
    let pool = [1usize, 2, 4, 8, 16];
    let m = pool[(rng.next() as usize) % pool.len()];
    let n = pool[(rng.next() as usize) % pool.len()];
    let k = pool[(rng.next() as usize) % pool.len()];
    let mode = rng.next() & 1;
...
mod tests {
    ...
    #[test]
    fn directed_ws_4x4_mode() {
        let case = gen_case(0, 1, 1);
        assert_eq!(case.cmd.mode, 1);
    }
    ...
    fn rs1_rs2_encoding() {
        ...
        assert_eq!(rs2, model::encode_rs2(4, 4, 4, 0));
    }
