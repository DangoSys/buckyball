...
class SystolicArrayStore(val b: GlobalConfig) extends Module {
  ...
  when(descriptorDeq) {
    assert(bufferCount =/= 0.U, "SystolicArrayStore: completed without a result row")
    assert(descriptorCount =/= 0.U, "SystolicArrayStore: completed without a write descriptor")
  }
}
