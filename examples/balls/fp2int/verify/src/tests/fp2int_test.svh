class fp2int_ball_test extends uvm_test;
  ...
  task run_case(int unsigned idx, string label, int unsigned bid);
    ...
    while (!env.scb.done()) begin
      ...
      if (cycles > FP2INT_TIMEOUT_CYCLES) begin
        `uvm_fatal(
            "TIMEOUT",
            $sformatf(
                "Fp2IntBall %s (idx=%0d) timeout: cmds=%0d reads=%0d/%0d writes=%0d/%0d resp=%0d",
    ...
    `uvm_info("FP2INT", $sformatf("Fp2IntBall %s (idx=%0d) passed in %0d cycles", label, idx,
                                  cycles), UVM_LOW)
  endtask
