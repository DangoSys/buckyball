...
#define BB_SMATMUL_CFG(rows, cols, first, last)                                \
  (FIELD((rows), 0, 11) | FIELD((cols), 12, 23) | FIELD((first), 24, 24) |     \
   FIELD((last), 25, 25))

#define bb_smatmul_bias(bias_bank)                                             \
  BUCKYBALL_INSTRUCTION_R_R(BB_BANK0(bias_bank) | BB_ITER(4), 0,               \
                            BB_FUNC7(SMATMUL_BIAS))
...
#define bb_smatmul_os(a_bank, b_bank, c_bank, rows, cols, k, first, last)      \
  BUCKYBALL_INSTRUCTION_R_R(                                                   \
      ...
      BB_SMATMUL_CFG(rows, cols, first, last), BB_FUNC7(SMATMUL_OS))
