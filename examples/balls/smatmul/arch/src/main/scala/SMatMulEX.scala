...
class SystolicArrayEX(b: GlobalConfig) extends SystolicArrayEXDatapath(b) {
  ...
  for (slot <- 0 until operandSlotCount) {
    ...
    val osLastUseCycle    =
      slotKBase(slot) + slotValidK(slot).pad(progressWidth) - 1.U +
    ...
    val reachedLastUse    =
      pipelineAdvance && slotContextActive && slotContextAge >= lastUseCycle

    when(slotOccupied(slot) && reachedLastUse) {
      slotUseDone(slot) := true.B
    }
    when(
      slotOccupied(slot) && slotInputComplete(slot) &&
  ...
  val segmentContext     = segmentOrder.io.deq
    .bits(contextWidth + operandSlotWidth - 1, operandSlotWidth)
  ...
  val activeOsInputsDrained = (0 until contextCount)
    .map { context =>
      !contextActive(context) || (!contextWsMode(context) && contextFinalSeen(
        context
      ) &&
  ...
  slotLaunching             := VecInit(
    (0 until operandSlotCount).map(slot => launchSegment && segmentSlot === slot.U)
  )

  when(launchSegment) {
    assert(
      slotContext(segmentSlot) === segmentContext,
  ...
  val wsBufferedFinal =
    wsBufferedResult.kTileKind === SystolicKTileKind.DIRECT ||
  ...
  wsResultBuffer.io.deq.ready := Mux(
    wsBufferedFinal,
    wsBufferedMatchesOutput && io.ex_st_o.ready,
    true.B
  )
  ...
  when(wsResultCommit) {
    ...
    assert(
      contextAllocated(commitContext) && contextWsMode(commitContext),
      "SystolicArrayEX: WS accumulator result refers to an invalid context"
    )
    assert(
      contextWsSegmentPending(commitContext),
      "SystolicArrayEX: WS accumulator result has no pending segment"
    )
    assert(
      commitRow === committedRows(rowIndexWidth - 1, 0),
      "SystolicArrayEX: WS accumulator rows committed out of order"
    )

    when(!wsBufferedFinal) {
      wsAccMem.write(
        wsAccAddress(commitContext, commitRow),
        wsBufferedResult.data
      )
    }.otherwise {
      contextRowsComplete(commitContext) := contextRowsComplete(
        commitContext
      ) + 1.U
    }
  ...
  when(io.ex_st_o.fire) {
    when(finishingOutput) {
      ...
      when(!contextWsMode(outputContext)) {
        assert(
          osAccBankAllocated(outputOsAccBank),
          "SystolicArrayEX: released an unallocated OS accumulator bank"
        )
        assert(
          osAccBankOwner(outputOsAccBank) === outputContext,
  ...
  when(firstReceiveEvent) {
    ...
    when(io.load_ex_req_kind === SystolicCtrlLoadReqKind.READ_A_B_PE) {
      assert(
        io.load_ex_b_valid_n === io.load_ex_valid_n &&
          io.load_ex_b_valid_k === io.load_ex_valid_k,
        "SystolicArrayEX: direct PE weights must match the current tile extent"
      )
    }
    when(
      firstIsContinuation && io.load_ex_req_kind === SystolicCtrlLoadReqKind.READ_AB
    ) {
      assert(
        chainValid,
        "SystolicArrayEX: continuation arrived without an active K-tile chain"
      )
    }
  }
  ...
  when(receiveActive) {
    assert(
      slotARowsReceived(receiveSlot) <= slotValidM(receiveSlot),
      "SystolicArrayEX: A receive row index overflow"
    )
    assert(
      slotBRowsReceived(receiveSlot) <=
  ...
  for (slot    <- 0 until operandSlotCount) {
    assert(
      slotARowsReceived(slot) <= slotValidM(slot),
      "SystolicArrayEX: A operand slot row count overflow"
    )
    assert(
      slotBRowsReceived(slot) <= Mux(
        needOp2(slotReqKind(slot)),
        slotBValidK(slot),
        0.U
      ),
    ...
    when(slotOccupied(slot) && slotInputComplete(slot)) {
      ...
      assert(
        !needOp2(slotReqKind(slot)) || slotBRowsReceived(slot) === slotBValidK(
          slot
        ),
    ...
    when(slotOccupied(slot) && !slotUseDone(slot)) {
      assert(
        contextAllocated(slotContext(slot)),
  ...
  when(outputOrder.io.deq.valid) {
    assert(
      contextAllocated(outputOrder.io.deq.bits),
