...
abstract class SystolicArrayEXBase(val b: GlobalConfig) extends Module {
  ...
  val contextTotalK = RegInit(
    VecInit(Seq.fill(contextCount)(0.U(progressWidth.W)))
  )

  val contextAge = RegInit(
    VecInit(Seq.fill(contextCount)(0.U(progressWidth.W)))
  )
  ...
  val contextActiveSlot = RegInit(
    VecInit(Seq.fill(contextCount)(0.U(operandSlotWidth.W)))
  )

  val contextWsMode = RegInit(VecInit(Seq.fill(contextCount)(false.B)))

  val contextWeightGeneration = RegInit(
    VecInit(Seq.fill(contextCount)(false.B))
  )

  val contextPendingWeightGeneration = RegInit(
    VecInit(Seq.fill(contextCount)(false.B))
  )

  val wsContextMap = RegInit(
    VecInit(Seq.tabulate(contextCount)(_.U(contextWidth.W)))
  )

  val wsContextMapValid = RegInit(VecInit(Seq.fill(contextCount)(false.B)))

  val contextOsAccBank = RegInit(
    VecInit(Seq.fill(contextCount)(0.U(osAccBankWidth.W)))
  )

  val contextWsSegmentPending = RegInit(
    VecInit(Seq.fill(contextCount)(false.B))
  )

  val contextWsRowsCommitted = RegInit(
    VecInit(Seq.fill(contextCount)(0.U(5.W)))
  )

  val contextWsKTileKind = RegInit(
    VecInit(Seq.fill(contextCount)(SystolicKTileKind.DIRECT))
  )
  ...
  val osAcc = RegInit(
    VecInit(
      Seq.tabulate(osAccBankCount)(_ =>
        VecInit(
          Seq.tabulate(tile)(_ => VecInit(Seq.fill(tile)(0.U(accElemBits.W))))
        )
      )
    )
  )
  ...
  val osAccBankOwner = RegInit(
    VecInit(Seq.fill(osAccBankCount)(0.U(contextWidth.W)))
  )
  ...
  val slotContext = RegInit(
    VecInit(Seq.fill(operandSlotCount)(0.U(contextWidth.W)))
  )

  val slotReqKind = RegInit(
    VecInit(Seq.fill(operandSlotCount)(SystolicCtrlLoadReqKind.READ_AB))
  )
  ...
  val slotKTileKind = RegInit(
    VecInit(Seq.fill(operandSlotCount)(SystolicKTileKind.DIRECT))
  )

  val slotKBase = RegInit(
    VecInit(Seq.fill(operandSlotCount)(0.U(progressWidth.W)))
  )

  val slotWeightGeneration = RegInit(
    VecInit(Seq.fill(operandSlotCount)(false.B))
  )
  ...
  val aRowValid = RegInit(
    VecInit(
      Seq.tabulate(operandSlotCount)(_ => VecInit(Seq.fill(tile)(false.B)))
    )
  )

  val bRowValid = RegInit(
    VecInit(
      Seq.tabulate(operandSlotCount)(_ => VecInit(Seq.fill(tile)(false.B)))
    )
  )

  val bPipeData = RegInit(
    VecInit(Seq.tabulate(tile)(_ => VecInit(Seq.fill(tile)(0.U(opElemBits.W)))))
  )

  val bPipeValid = RegInit(
    VecInit(Seq.tabulate(tile)(_ => VecInit(Seq.fill(tile)(false.B))))
  )

  val bPipeContext = RegInit(
    VecInit(
  ...
  val segmentOrder = Module(
    new Queue(UInt((contextWidth + operandSlotWidth).W), operandSlotCount)
  )
