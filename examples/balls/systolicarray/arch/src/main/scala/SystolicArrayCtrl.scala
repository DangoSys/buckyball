...
class SystolicArrayCtrl(val b: GlobalConfig) extends Module {
  ...
  val storeRespBits  = RegInit(0.U.asTypeOf(new SystolicStoreCtrlResp(b)))
  ...
  currentKind := Mux(
    task.mode,
    Mux(wsLoadPeB, SystolicCtrlLoadReqKind.READ_A_B_PE, SystolicCtrlLoadReqKind.READ_A_ONLY),
  ...
  val currentInputRows = MuxLookup(currentKind, currentValidM)(Seq(
    SystolicCtrlLoadReqKind.READ_AB -> Mux(currentValidM >= currentValidK, currentValidM, currentValidK),
    SystolicCtrlLoadReqKind.READ_A_ONLY -> currentValidM,
    SystolicCtrlLoadReqKind.READ_A_B_PE -> tile.U(5.W)
  ))
  ...
  when(io.store_req_i) {
    assert(state === sRun, "SystolicArrayCtrl: store_req arrived outside task execution")
    assert(!storeRespValid, "SystolicArrayCtrl: store_req arrived while previous store response is still pending")
    assert(!storeRespAccepted, "SystolicArrayCtrl: store_req arrived before previous store row completed")
  ...
  when(io.store_done_i) {
    assert(storeRespAccepted, "SystolicArrayCtrl: store_done without an accepted store response")
