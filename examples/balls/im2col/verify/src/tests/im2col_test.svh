class im2col_ball_test extends uvm_test;
  ...
  function void build_phase(uvm_phase phase);
    ...
    env = im2col_env::type_id::create("env", this);
  endfunction

  task run_phase(uvm_phase phase);
    int unsigned bid;
    bid = im2col_require_bid();
  ...
  task run_directed(int unsigned bid);
    run_case(0, "DIR_K3_PAD0", bid);
    run_case(1, "DIR_K3_PAD1", bid);
  endtask
  ...
  task run_case(int unsigned idx, string label, int unsigned bid);
    im2col_basic_seq seq;
    int cycles;

    apply_reset();
    env.scb.reset_counters();

    seq = im2col_basic_seq::type_id::create("seq");
    seq.case_index = idx;
    seq.bid = bid;
    ...
    while (!env.scb.done()) begin
      @(posedge vif.clock);
      cycles++;
      if (cycles > IM2COL_TIMEOUT_CYCLES) begin
        `uvm_fatal("TIMEOUT",
                   $sformatf(
                       "Im2colBall %s (idx=%0d) timeout: cmds=%0d reads=%0d wr=%0d/%0d resp=%0d",
                       label, idx, env.scb.cmd_count, env.scb.read_count, env.scb.write_count,
                       env.scb.expected_writes, env.scb.resp_count))
      end
    end

    `uvm_info("IM2COL", $sformatf("Im2colBall %s (idx=%0d) passed in %0d cycles", label, idx,
                                  cycles), UVM_LOW)
  endtask
