...
static void issue_accumulated_matmul(int ws_mode) {
  ...
  if (ws_mode) {
    bb_matrix_ws_ACC_FIRST(op1_bank_id, op2_bank_id, acc_bank_id, TILE_DIM);
  } else {
    bb_matrix_os_ACC_FIRST(op1_bank_id, op2_bank_id, acc_bank_id, TILE_DIM);
  }

  bb_mvin((uintptr_t)packed_a_tiles[1], op1_bank_id, TILE_DIM, 1);
  bb_mvin((uintptr_t)packed_b_tiles[1], op2_bank_id, TILE_DIM, 1);
  if (ws_mode) {
    bb_matrix_ws_ACC_LAST(op1_bank_id, op2_bank_id, acc_bank_id, TILE_DIM);
  } else {
    bb_matrix_os_ACC_LAST(op1_bank_id, op2_bank_id, acc_bank_id, TILE_DIM);
  }
...
int main() {
  ...
  int os_passed = run_test("matrix_matmul_4x8_8x4_os", 0);
  int ws_passed = run_test("matrix_matmul_4x8_8x4_ws", 1);
