...
pub fn run_im2col(
  ...
  if blink.sram_read_resp[0].valid {
    compute.store_input(compute.resp_counter as usize, &blink.sram_read_resp[0].data);
    compute.resp_counter += 1;
    if compute.is_read_done() { compute.convert_im2col(); }
  }
  ...
  if compute.is_read_done() && compute.should_write() {
    ...
    blink.sram_write_req[0].data = compute.get_output_row(compute.write_counter as usize);
