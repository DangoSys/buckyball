...
public:
  ...
  void runOnOperation() override {
    ...
    // Walk all ops including those nested in scf.for / scf.if bodies so we
    // process bank ops that appear inside loop bodies generated by Tile pass.
    // PreOrder walk preserves source order, so alloc/release pairing stays
    // correct. SmallVector buffer is needed because we erase ops during the
    // walk.
    ...
    for (Operation *opPtr : opsToProcess) {
      Operation &op = *opPtr;
      ...
      if (auto q = dyn_cast<buckyball::BankFp2IntOp>(op)) {
        auto in = getConstI64(q.getInBank());
        auto out = getConstI64(q.getOutBank());
        auto iter = getConstI64(q.getIter());
        if (!in || !out || !iter) {
          op.emitError(
              "bank_fp2int expects constant in/out/iter after assignment");
          return signalPassFailure();
        }
        uint64_t rs1 = fieldBits(*in, 0, 9) | fieldBits(*out, 20, 29) |
                       fieldBits(*iter, 30, 63);
      ...
      if (auto dq = dyn_cast<buckyball::BankInt2FpOp>(op)) {
        auto in = getConstI64(dq.getInBank());
        auto out = getConstI64(dq.getOutBank());
        auto iter = getConstI64(dq.getIter());
        if (!in || !out || !iter) {
          op.emitError(
              "bank_int2fp expects constant in/out/iter after assignment");
          return signalPassFailure();
        }
        uint64_t rs1 = fieldBits(*in, 0, 9) | fieldBits(*out, 20, 29) |
                       fieldBits(*iter, 30, 63);
