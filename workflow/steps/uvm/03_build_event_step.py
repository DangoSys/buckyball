...
config = {
  ...
  "subscribes": ["uvm.build"],
  "emits": ["uvm.sim", "uvm.complete", "uvm.error"],
  "flows": ["uvm"],
}
...
# ==================================================================================
  ...
  csrcs = (glob.glob(f"{dut_dir}/src/main/csrc/**/*.c", recursive=True) + 
           glob.glob(f"{dut_dir}/src/main/csrc/**/*.cc", recursive=True) +
           glob.glob(f"{dut_dir}/src/main/csrc/**/*.cpp", recursive=True) +
  ...
  inc_paths = [
    ...
    build_dir,
    f"{dut_dir}/src/main/csrc/include"
  ]
  ...
  result = stream_run_logger(cmd=verilator_cmd, logger=context.logger, cwd=bbdir, stdout_prefix="uvm build", stderr_prefix="uvm build")
