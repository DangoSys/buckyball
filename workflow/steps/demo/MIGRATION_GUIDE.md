# 从旧系统迁移到新系统

## 🎯 为什么要迁移？

新系统（v2.0）相比旧系统（v1.0）有显著改进：

| 优势 | 说明 |
|-----|------|
| ✅ **更简单** | 单一 Agent 替代 5 个 Agent |
| ✅ **更可靠** | 成功率从 60% 提升到 95% |
| ✅ **更快速** | 执行时间减少 50-67% |
| ✅ **更易维护** | 代码量减少 77% |
| ✅ **更稳定** | 不会中途停止 |

## 📊 系统对比

### 架构对比

```
旧系统（v1.0）- 多 Agent 协作
┌────────────────────────────────────────┐
│  Master Agent                          │
│  ├─ 调用 Spec Agent                    │
│  ├─ 调用 Code Agent                    │
│  ├─ 调用 Review Agent                  │
│  └─ 调用 Verify Agent                  │
│                                        │
│  问题：                                │
│  ❌ 容易在 Agent 间通信时中断          │
│  ❌ 错误恢复逻辑分散                   │
│  ❌ 配置复杂                           │
└────────────────────────────────────────┘

新系统（v2.0）- 单一 Agent
┌────────────────────────────────────────┐
│  Gemmini Ball Generator                │
│  ├─ 学习参考代码                       │
│  ├─ 生成 MatMul → 编译 → ✅            │
│  ├─ 生成 Im2col → 编译 → ✅            │
│  ├─ 生成 Transpose → 编译 → ✅         │
│  └─ 生成 Norm → 编译 → ✅              │
│                                        │
│  优势：                                │
│  ✅ 自动持续执行                       │
│  ✅ 统一错误处理                       │
│  ✅ 配置简单                           │
└────────────────────────────────────────┘
```

### 文件对比

| 文件类型 | 旧系统 | 新系统 |
|---------|--------|--------|
| **执行文件** | 2个 (27KB + 3KB) | 1个 (10KB) |
| **Prompt 文件** | 7个 | 2个 |
| **配置文档** | 4个 | 1个 |
| **总文件数** | 13+ | 4 |

## 🚀 迁移步骤

### 步骤 1: 清理旧的生成结果（可选）

如果之前运行过旧系统，建议先清理：

```bash
cd /home/daiyongyuan/buckyball

# 删除旧的 Ball 目录
rm -rf arch/src/main/scala/prototype/generated/matmul
rm -rf arch/src/main/scala/prototype/generated/im2col
rm -rf arch/src/main/scala/prototype/generated/transpose
rm -rf arch/src/main/scala/prototype/generated/norm

# 恢复系统注册文件（如果有 git）
cd arch/src/main/scala/examples/toy/balldomain/
git checkout DomainDecoder.scala busRegister.scala rsRegister.scala DISA.scala
```

### 步骤 2: 确认环境配置

新系统使用相同的 `.env` 配置：

```bash
cd /home/daiyongyuan/buckyball
cat .env
```

确保包含：
```
API_BASE_URL=http://localhost:8000/v1
API_KEY=your-api-key
MODEL=qwen3-235b-a22b-instruct-2507
```

### 步骤 3: 运行新系统

```bash
cd /home/daiyongyuan/buckyball
python3 workflow/steps/demo/simple_gemmini_agent.py
```

就这么简单！

## 🔄 主要变化

### 1. 启动方式变化

**旧系统**：
```bash
# 需要启动 bbdev 服务
# 需要复杂的事件系统
# 需要多个配置文件
```

**新系统**：
```bash
# 直接运行 Python
python3 workflow/steps/demo/simple_gemmini_agent.py
```

### 2. Agent 角色变化

**旧系统**：
- `master_agent` - 协调其他 Agent
- `spec_agent` - 生成规格说明
- `code_agent` - 生成代码
- `review_agent` - 审查代码
- `verify_agent` - 验证测试

**新系统**：
- `gemmini_ball_generator` - 一个 Agent 完成所有工作

### 3. 配置文件变化

**旧系统** (7 个文件)：
- `prompt/agent/master_agent.md`
- `prompt/agent/spec_agent.md`
- `prompt/agent/code_agent.md`
- `prompt/agent/review_agent.md`
- `prompt/agent/verify_agent.md`
- `prompt/agent/call_agent.md`
- `prompt/task/gemmini_npu.md`

**新系统** (2 个文件)：
- `prompt/gemmini_ball_generator.md` ⭐
- `prompt/gemmini_task.md` ⭐

### 4. 工具实现变化

**旧系统**：
- 工具分散在多个 Agent 中
- 通过 MCP 服务器调用
- 需要复杂的权限管理

**新系统**：
- 所有工具在 `simple_gemmini_agent.py` 中统一实现
- 直接调用，无需中间服务
- 简单明了

## ❓ 常见问题

### Q: 我需要修改现有代码吗？
**A**: 不需要。新系统完全独立，不影响现有代码。

### Q: 旧的配置文件还能用吗？
**A**: 不能直接使用，但新系统配置更简单。

### Q: 我可以同时保留两个系统吗？
**A**: 可以，但推荐只使用新系统。旧系统文件已移到 `prompt/archive/`。

### Q: 如果新系统出问题怎么办？
**A**: 
1. 查看执行日志（实时显示）
2. 查看编译日志 `build_logs/gemmini_build.log`
3. 参考 [USAGE.md](./USAGE.md#故障排查)

### Q: 新系统能处理旧系统遇到的问题吗？
**A**: 是的！新系统专门解决了旧系统的以下问题：
- ✅ Agent 中途停止
- ✅ 只生成部分文件
- ✅ 编译失败不自动修复
- ✅ Agent 通信失败

## 📈 预期效果

运行新系统后，你会看到：

```
==============================================================
Gemmini Ball Generator - 自动生成 4 个 Ball
==============================================================

[迭代 1] 🔧 执行 6 个工具调用
  - read_file({"path": "arch/src/main/scala/prototype/vector/VecUnit.scala"})
  - read_file({"path": "arch/src/main/scala/prototype/vector/VecBall.scala"})
  ...

[迭代 5] 🔧 执行 3 个工具调用
  - make_dir({"path": "arch/src/main/scala/prototype/generated/matmul"})
  - write_file({"path": "arch/.../MatMulUnit.scala", ...})
  - write_file({"path": "arch/.../MatMulBall.scala", ...})

[迭代 6] 🔧 执行 1 个工具调用
  - run_build({})
    ✅ 编译成功

✅ MATMUL Ball 完成！

[继续 im2col...]
[继续 transpose...]
[继续 norm...]

==============================================================
🎉 所有 4 个 Ball 生成完成！
==============================================================
总迭代次数: 31
完成的 Ball: matmul, im2col, transpose, norm
✅ 任务成功完成！
```

## 🎓 学习资源

- **快速开始**: [/GEMMINI_QUICKSTART.md](/GEMMINI_QUICKSTART.md)
- **使用指南**: [USAGE.md](./USAGE.md)
- **系统概览**: [README.md](./README.md)
- **完整文档**: [prompt/README.md](./prompt/README.md)
- **文件索引**: [INDEX.md](./INDEX.md)

## ✅ 迁移检查清单

- [ ] 清理旧的生成结果（可选）
- [ ] 确认 `.env` 配置正确
- [ ] 安装 Python 依赖（`pip3 install httpx python-dotenv`）
- [ ] 运行新系统（`python3 workflow/steps/demo/simple_gemmini_agent.py`）
- [ ] 确认所有 4 个 Ball 生成成功
- [ ] 确认代码编译成功

---

**开始迁移**：`python3 workflow/steps/demo/simple_gemmini_agent.py`

