  ...
  },
  {
    ...
    "function": {
      ...
      "parameters": {
        "type": "object",
        "properties": {
          "test_file": {"type": "string", "description": "C 测试文件路径（如 tests/gemmini_abft_test.c）"}
        },
        "required": ["test_file"]
      }
    }
...
def execute_tool(tool_name: str, arguments: Dict[str, Any]) -> str:
  ...
  try:
    if tool_name == "read_file":
      ...
      test_path = WORK_DIR / test_file if not test_file.startswith("/") else Path(test_file)
          ...
          try:
            ...
            if func_name == "write_file":
              if consecutive_json_errors >= 3:
                error_msg = f"""Error: JSON parsing failed 3 times in a row!
...
Please try a different approach."""
              ...
              error_msg = f"Error: JSON parsing failed - {str(e)}. Please simplify your arguments and try again."
          ...
          # 检查是否是测试结果
            ...
            try:
              ...
              if test_result["status"] == "success":
                ...
                print(f"    ❌ 测试失败，需要修复")
                print(f"    输出: {test_result.get('stdout', '')[:200]}")
              elif test_result["status"] == "compile_failed":
                print(f"    ❌ C 测试编译失败")
                print(f"    错误: {test_result.get('stderr', '')[:200]}")
      ...
      # 检查是否完成
        ...
        if last_build_success and any(kw in content.lower() for kw in ["完成", "成功", "finished", "done"]):
