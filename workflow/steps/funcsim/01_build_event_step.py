...
async def handler(data, context):
    ...
    if not os.path.exists(customext_dir):
        context.logger.error(
            "customext directory not found", {"customext_dir": customext_dir}
        )
    command = f"source {bbdir}/env.sh && cd {customext_dir} && mkdir -p build && cd build && cmake .. && make install"
    context.logger.info(
        "Executing funcsim command", {"command": command, "cwd": customext_dir}
    )
    result = stream_run_logger(
        ...
        cwd=customext_dir,
        executable="bash",
        stdout_prefix="build customext",
        stderr_prefix="build customext",
    )
    ...
    result = stream_run_logger(
        ...
        cwd=funcsim_dir,
        executable="bash",
        stdout_prefix="build funcsim",
        stderr_prefix="build funcsim",
    )
