...
def setup_before_all_tests():
  ...
  for idx, (workload_path, workload_id) in enumerate(ctest_workloads):
    ...
    processes.append((process, port, workload_id))
  ...
  for process, port, workload_id in processes:
    logging.info(f"停止服务 {workload_id} (端口 {port})")
...
def test_ctest_workload_debug(script_runner, caplog, workload_path, workload_id, test_index):
  ...
  result = subprocess.run(command, shell=True, timeout=60000, capture_output=True, text=True)
