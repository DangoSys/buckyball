...
const InstructionConfig conv_config = {
    ...
    .rs2_fields = (BitFieldConfig[]){
        {"ofmap_spaddr", 0, 14},
        {"iter", 15, 24},
        // special field is 40 bits: rs2(63, spAddrLen + 10) = rs2(63, 24) = 40 bits
        // Encode: in_height[15:0], in_width[31:16], kernel_h[39:32], kernel_w[47:40] but only 40 bits available
        // Adjust: use in_height[15:0], in_width[31:16], kernel_h[39:32], kernel_w same as kernel_h
        {"in_height", 25, 40},
        {"in_width", 41, 56},
        {"kernel_h", 57, 64},
        {NULL, 0, 0}}};
...
// Encode in special: in_height[15:0] = special[15:0], in_width[15:0] = special[31:16], kernel_h[7:0] = special[39:32]
// kernel_w is assumed to equal kernel_h for simplicity
...
void bb_conv(uint32_t ifmap_addr, uint32_t weight_addr, uint32_t ofmap_addr,
  ...
  uint64_t rs2_val =
      CONV_ENCODE_RS2(ofmap_addr, iter, in_height, in_width, kernel_h, kernel_w);
