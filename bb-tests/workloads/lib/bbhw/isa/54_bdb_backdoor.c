...
#define bdb_backdoor_write(bank_id, iter)                                      \
  BUCKYBALL_INSTRUCTION_R_R((BB_BANK2(bank_id) | BB_ITER(iter)), 0,            \
                            BDB_BACKDOOR_FUNC7)
...
#define bdb_backdoor_read(bank_id, iter)                                       \
  BUCKYBALL_INSTRUCTION_R_R((BB_BANK0(bank_id) | BB_ITER(iter)), 0,            \
                            BDB_BACKDOOR_FUNC7)

// Backdoor peek: read single row from external bank
#define bdb_backdoor_peek(bank_id, row_count)                                  \
  BUCKYBALL_INSTRUCTION_R_R((BB_BANK0(bank_id) | BB_ITER(row_count)), 0,       \
                            BDB_BACKDOOR_FUNC7)
