...
// 运行时接口 - 纯 C 实现
static inline uint32_t spad_addr(uint32_t bank, uint32_t row) {
  return (bank < BANK_NUM) ? bank_addr(&bank_configs[bank], row) : -1;
}

// 从spad地址获取bank编号
static inline uint32_t spad_get_bank(uint32_t addr) {
...
static inline uint32_t spad_get_offset(uint32_t addr) {
  for (uint32_t bank = 0; bank < BANK_NUM; bank++) {
    uint32_t base = bank_configs[bank].base_addr_;
    uint32_t size = bank_configs[bank].row_num_;
    if (addr >= base && addr < base + size) {
...
static inline void spad_get_bank_offset(uint32_t addr, uint32_t *bank,
                                        uint32_t *offset) {
...
// 获取指定bank的行宽度（字节数）
static inline uint32_t spad_get_bank_row_bytes(uint32_t bank) {
  if (bank >= BANK_NUM)
    return 0;
