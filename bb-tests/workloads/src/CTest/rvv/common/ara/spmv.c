...
void spmv_csr_idx32(int32_t N_ROW, int32_t *CSR_PROW, int32_t *CSR_INDEX,
  ...
  for (int i = 0; i < N_ROW; ++i) {
    ...
    while (len) {
      ...
      /* 	  printf("i=%ld j=%ld id=%ld data=%lx vec=%lx out=%lx
       * vdata=\"%lx\" vvec=\"%lx\" vout=\"%lx\"\n", */
      /* 		 i, j, index[j] / sizeof(double), */
      /* 		 *(uint64_t*)(&data[j]),
       * *(uint64_t*)(&IN_VEC[index[j]/sizeof(double)]), */
      /* 		 *(uint64_t*)(&dbg), */
