  ...
  for (uint64_t i = 0; i < N_f64; ++i) {
    if (!similarity_check(results[i], gold_results_f64[i], THRESHOLD)) {
      ...
      printf("64-bit error at index %d. %lx != %lx\n", i,
             *(uint64_t *)(&results[i]), *(uint64_t *)(&gold_results_f64[i]));
    }
  }
  return error;
}
  ...
  for (uint64_t i = 0; i < N_f32; ++i) {
    if (!similarity_check(results[i], gold_results_f32[i], THRESHOLD)) {
      ...
      printf("32-bit error at index %d. %x != %x\n", i,
             *(uint32_t *)(&results[i]), *(uint32_t *)(&gold_results_f32[i]));
    }
  }
  return error;
}
...
int main() {
  ...
  error = check64(results_f64m1);
  if (error) {
    return error;
  }
  error = check64(results_f64m2);
  if (error) {
    return error;
  }
  error = check64(results_f64m4);
  if (error) {
    return error;
  }
  error = check32(results_f32m1);
  if (error) {
    return error;
  }
  error = check32(results_f32m2);
  if (error) {
    return error;
  }
  error = check32(results_f32m4);
  if (error) {
    return error;
  }
