...
void handle_fault(uintptr_t addr, uintptr_t cause) {
  ...
  uintptr_t new_pte = (node->addr >> PGSHIFT << PTE_PPN_SHIFT) | PTE_V | PTE_U |
                      PTE_R | PTE_W | PTE_X;
...
static void coherence_torture() {
  ...
  while (1) {
    uintptr_t paddr =
        DRAM_BASE + ((random % (2 * (MAX_TEST_PAGES + 1) * PGSIZE)) & -4);
...
#if SATP_MODE_CHOICE == SATP_MODE_SV48
  l1pt[PTES_PER_PT - 1] =
      ((pte_t)kernel_l2pt >> PGSHIFT << PTE_PPN_SHIFT) | PTE_V;
  kernel_l2pt[PTES_PER_PT - 1] =
      ((pte_t)kernel_l3pt >> PGSHIFT << PTE_PPN_SHIFT) | PTE_V;
...
#error
  ...
  write_csr(medeleg, (1 << CAUSE_USER_ECALL) | (1 << CAUSE_FETCH_PAGE_FAULT) |
