        ...
        if y % strides == 0:
            for x in range(image.shape[0]):
                ...
                try:
                    ...
                    if x % strides == 0:
                        output[x, y] = (
                            kernel * image[x : x + xKernShape, y : y + yKernShape]
                        ).sum()
                except Exception:
                    break
...
if len(sys.argv) > 1:
    ...
    assert (
        matrix_width <= 128
    ), "The width of the image cannot be greater than 128 64-bit \
...
assert (
    M % 4 == 0
), "Output image dimension must be divisible by 4, pad the input image accordingly"
assert (
    N % 4 == 0
), "Output image dimension must be divisible by 4, pad the input image accordingly"

# Generate a random int64 input padded image
image = np.random.randint(MIN_DTYPE, MAX_DTYPE, M_pad * N_pad, dtype).reshape(
    M_pad, N_pad
)

# Generate a random int64 filter
gen_filter = np.random.randint(MIN_DTYPE, MAX_DTYPE, F * F, dtype).reshape(F, F)
