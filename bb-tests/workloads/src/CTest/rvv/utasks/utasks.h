...
public:
  region_allocator_t(void *base, size_t size)
      : base(base), size(size), tail(0) {}
  void *allocate(size_t n) {
    lock.lock();
    if (tail + n > size) {
      lock.unlock();
...
public:
  ...
  bool __attribute__((noinline)) is_finished() {
    return may_finish && !has_work();
  }
...
template <typename T> class alignas(64) circular_buffer_t {
public:
  circular_buffer_t(size_t capacity, void *buffer,
                    circular_buffer_pointers_t *pointers)
      : buffer((T *)buffer), capacity(capacity), pointers(pointers) {
  ...
  ~circular_buffer_t() { delete[] buffer; }
  ...
  std::pair<T *, size_t> push(size_t n) {
    ...
    asm volatile("amoadd.d %[rd], %[incr], (%[addr])\n"
                 : [rd] "=r"(tail_wide_read)
                 : [incr] "r"(n), [addr] "r"(&pointers->tail_wide));
    asm volatile("amoadd.d %[rd], %[incr], (%[addr])\n"
                 : [rd] "=r"(size_read)
                 : [incr] "r"(n), [addr] "r"(&pointers->size));
    tail_wide_read += n;
    size_read += n;
  ...
  std::pair<T *, size_t> pop(size_t n) {
    ...
    asm volatile("amoadd.d %[rd], %[incr], (%[addr])\n"
                 : [rd] "=r"(head_wide_read)
                 : [incr] "r"(n), [addr] "r"(&pointers->head_wide));
    asm volatile("amoadd.d %[rd], %[incr], (%[addr])\n"
                 : [rd] "=r"(size_read)
                 : [incr] "r"((~n) + 1), [addr] "r"(&pointers->size));
    head_wide_read += n;
    size_read -= n;
...
template <typename T, typename U> class circular_buffer_helper_t {
public:
  static std::tuple<T *, U *, size_t>
  push_pop(circular_buffer_t<T> *const source,
           circular_buffer_pointers_t *source_pointers,
           circular_buffer_t<U> *const sink,
           circular_buffer_pointers_t *sink_pointers, size_t n) {
    size_t sink_tail_wide_read, sink_size_read;
    size_t source_head_wide_read, source_size_read;
    asm volatile("amoadd.d %[rd], %[incr], (%[addr])\n"
                 : [rd] "=r"(sink_tail_wide_read)
                 : [incr] "r"(n), [addr] "r"(&sink_pointers->tail_wide));
    asm volatile("amoadd.d %[rd], %[incr], (%[addr])\n"
                 : [rd] "=r"(source_head_wide_read)
                 : [incr] "r"(n), [addr] "r"(&source_pointers->head_wide));
    asm volatile("amoadd.d %[rd], %[incr], (%[addr])\n"
                 : [rd] "=r"(sink_size_read)
                 : [incr] "r"(n), [addr] "r"(&sink_pointers->size));
    asm volatile("amoadd.d %[rd], %[incr], (%[addr])\n"
                 : [rd] "=r"(source_size_read)
                 : [incr] "r"((~n) + 1), [addr] "r"(&source_pointers->size));
    ...
    asm volatile("add %[rd], %[src], %[incr]\n"
                 : [rd] "=r"(sink_tail_wide_read)
                 : [src] "r"(sink_tail_wide_read), [incr] "r"(n));
    asm volatile("add %[rd], %[src], %[incr]\n"
                 : [rd] "=r"(source_head_wide_read)
                 : [src] "r"(source_head_wide_read), [incr] "r"(n));
    asm volatile("add %[rd], %[src], %[incr]\n"
                 : [rd] "=r"(sink_size_read)
                 : [src] "r"(sink_size_read), [incr] "r"(n));
    asm volatile("sub %[rd], %[src], %[incr]\n"
                 : [rd] "=r"(source_size_read)
                 : [src] "r"(source_size_read), [incr] "r"(n));
    ...
    size_t sink_limit =
        ((sink_head > sink_tail) ? sink_head : sink_capacity) - sink_tail;
    size_t sink_r =
        (remaining_capacity > sink_limit) ? sink_limit : remaining_capacity;

    size_t source_head = source_head_wide_read & source_mask;
    size_t source_tail = source_tail_wide & source_mask;
    size_t source_limit =
        ((source_tail > source_head) ? source_tail : source_capacity) -
        source_head;
    size_t source_r =
        (source_size_read > source_limit) ? source_limit : source_size_read;

    size_t r = (sink_r > source_r) ? source_r : sink_r;
    return std::make_tuple(source_buffer + source_head, sink_buffer + sink_tail,
                           r);
  }
};
...
class runner_t {
public:
  runner_t(size_t id, allocator_t *allocator)
      : id(id), task_count(0), allocator(allocator) {};
  void run() {
    while (1) {
...
template <typename T> class sink_t : virtual public task_t {
...
public:
  ...
  void assign_runner(runner_t *runner) {
    this->runner = runner;
    void *pointers_buff =
        runner->get_allocator()->allocate(sizeof(circular_buffer_pointers_t));
    circular_buffer_pointers_t *pointers =
        new (pointers_buff) circular_buffer_pointers_t;
...
template <typename T> class source_t : virtual public task_t {
public:
...
public:
  pipe_task_t(size_t buffer_size, size_t max_chunk)
      : max_chunk(max_chunk), sink_t<T>(buffer_size) {}
...
private:
  void run() {
    ...
    if (has_next) {
      ...
      std::tuple<T *, T *, size_t> r = circular_buffer_helper_t<T, U>::push_pop(
          buffer, source_pointers, next_buffer, next_pointers, 0);
      T *input = std::get<0>(r);
      ...
      while (n > 0) {
        ...
        asm volatile("fence");
        r = circular_buffer_helper_t<T, U>::push_pop(
            buffer, source_pointers, next_buffer, next_pointers, completed);
        input = std::get<0>(r);
        output = std::get<1>(r);
        n = std::get<2>(r);
        if (completed == 0)
...
template <typename T> class source_task_t : public source_t<T> {
public:
...
template <typename T> class sink_task_t : public sink_t<T> {
public:
  sink_task_t(size_t buffer_size, size_t max_chunk)
      : max_chunk(max_chunk), sink_t<T>(buffer_size) {}
  bool has_work() { return this->buffer->busy(); }
