...
int test_col_row_vector() {
  ...
  return run_test("Column vector × Row vector", input_matrix_a, input_matrix_b,
                  DIM);
}
...
int main() {
  ...
  multicore(MULTICORE);
#endif
  int passed = test_col_row_vector();
  if (passed) {
    printf("vecunit_matmul_col_row_vector test PASSED\n");
    return 0;
  } else {
    printf("vecunit_matmul_col_row_vector test FAILED\n");
    return 1;
  }
#ifdef MULTICORE
  exit(0);
