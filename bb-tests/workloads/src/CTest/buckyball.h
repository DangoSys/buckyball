...
// #define SP_MATRICES ((BANK_NUM * BANK_ROWS) / DIM)  // Number of matrices in
// scratchpad
...
#define BB_MATMUL_WS_FUNCT                                                     \
  27 // 0x1B - Matrix multiply with warp16 function code
...
// mem_addr: DRAM address, sp_addr: scratchpad address, iter: number of times to
// transfer
#define bb_mvin(mem_addr, sp_addr, iter)                                       \
  do {                                                                         \
    uint64_t rs1_val = (uint64_t)(mem_addr);                                   \
    uint64_t rs2_val = ((iter) << SPAD_ADDR_LEN) |                             \
                       ((sp_addr) & ((1UL << SPAD_ADDR_LEN) - 1));             \
...
// mem_addr: DRAM address, sp_addr: scratchpad address, iter: number of times to
// transfer
#define bb_mvout(mem_addr, sp_addr, iter)                                      \
  do {                                                                         \
    uint64_t rs1_val = (uint64_t)(mem_addr);                                   \
    uint64_t rs2_val = ((iter) << SPAD_ADDR_LEN) |                             \
                       ((sp_addr) & ((1UL << SPAD_ADDR_LEN) - 1));             \
...
// op1_addr: first operand scratchpad address, op2_addr: second operand
// scratchpad address wr_addr: write result scratchpad address, iter: number of
// iterations
#define bb_mul_warp16(op1_addr, op2_addr, wr_addr, iter)                       \
  do {                                                                         \
    uint64_t rs1_val = ((op2_addr) << SPAD_ADDR_LEN) |                         \
                       ((op1_addr) & ((1UL << SPAD_ADDR_LEN) - 1));            \
    uint64_t rs2_val = ((iter) << SPAD_ADDR_LEN) |                             \
                       ((wr_addr) & ((1UL << SPAD_ADDR_LEN) - 1));             \
...
#define bb_matmul_ws(op1_addr, op2_addr, wr_addr, iter)                        \
  do {                                                                         \
    uint64_t rs1_val = ((op2_addr) << SPAD_ADDR_LEN) |                         \
                       ((op1_addr) & ((1UL << SPAD_ADDR_LEN) - 1));            \
    uint64_t rs2_val =                                                         \
        (((1) << (SPAD_ADDR_LEN + 10)) | (iter) << SPAD_ADDR_LEN) |            \
        ((wr_addr) & ((1UL << SPAD_ADDR_LEN) - 1));                            \
    BUCKYBALL_INSTRUCTION_R_R(rs1_val, rs2_val, BB_MATMUL_WS_FUNCT);           \
  } while (0)

#define bb_im2col(op1_addr, wr_addr, krow, kcol, inrow, incol, startrow,       \
                  startcol)                                                    \
  do {                                                                         \
    uint64_t rs1_val = ((wr_addr) << SPAD_ADDR_LEN) |                          \
                       ((op1_addr) & ((1UL << SPAD_ADDR_LEN) - 1));            \
    uint64_t rs2_val =                                                         \
        (((startrow) << (SPAD_ADDR_LEN + 38)) |                                \
         ((startcol) << (SPAD_ADDR_LEN + 33)) |                                \
         ((inrow) << (SPAD_ADDR_LEN + 23)) |                                   \
         ((incol) << (SPAD_ADDR_LEN + 18)) |                                   \
         ((krow) << (SPAD_ADDR_LEN + 14)) | ((kcol) << (SPAD_ADDR_LEN + 10))); \
...
// Flush accelerator
#define bb_flush() BUCKYBALL_INSTRUCTION_FLUSH(BB_FLUSH_FUNCT)
