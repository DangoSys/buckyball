...
#define IN_ROW_DIM_DILATED                                                     \
  (IN_ROW_DIM + (INPUT_DILATION - 1) * (IN_ROW_DIM - 1))
#define IN_COL_DIM_DILATED                                                     \
  (IN_COL_DIM + (INPUT_DILATION - 1) * (IN_COL_DIM - 1))
#define KERNEL_DIM_DILATED                                                     \
  (KERNEL_DIM + (KERNEL_DILATION - 1) * (KERNEL_DIM - 1))
#define OUT_ROW_DIM                                                            \
  ((IN_ROW_DIM_DILATED + 2 * PADDING - KERNEL_DIM_DILATED) / STRIDE + 1)
#define OUT_COL_DIM                                                            \
  ((IN_COL_DIM_DILATED + 2 * PADDING - KERNEL_DIM_DILATED) / STRIDE + 1)
#define PATCH_SIZE (KERNEL_DIM * KERNEL_DIM * IN_CHANNELS)
#define N_PATCHES (BATCH_SIZE * OUT_ROW_DIM * OUT_COL_DIM)

void conv(int batch_size, int in_channels, int in_row_dim, int in_col_dim,
  ...
  const size_t in_row_dim_dilated =
      in_row_dim + (input_dilation - 1) * (in_row_dim - 1);
  const size_t in_col_dim_dilated =
      in_col_dim + (input_dilation - 1) * (in_col_dim - 1);
  assert(in_row_dim_dilated == IN_ROW_DIM_DILATED);
  assert(in_col_dim_dilated == IN_COL_DIM_DILATED);
  static elem_t dilated[BATCH_SIZE][IN_ROW_DIM_DILATED][IN_COL_DIM_DILATED]
                       [IN_CHANNELS];

  const size_t kernel_dim_dilated =
      kernel_dim + (kernel_dilation - 1) * (kernel_dim - 1);
  assert(kernel_dim_dilated == KERNEL_DIM_DILATED);
  static elem_t weights_dilated[OUT_CHANNELS][KERNEL_DIM_DILATED]
                               [KERNEL_DIM_DILATED][IN_CHANNELS];
  ...
  for (int b = 0; b < batch_size; b++)
    for (int irow = 0; irow < in_row_dim_dilated; irow++)
      for (int icol = 0; icol < in_col_dim_dilated; icol++)
        for (int ich = 0; ich < in_channels; ich++)
          dilated[b][irow][icol][ich] = 0;

  size_t idx = 0;
  for (int b = 0; b < batch_size; b++)
    for (int irow = 0; irow < in_row_dim_dilated; irow += input_dilation)
      for (int icol = 0; icol < in_col_dim_dilated; icol += input_dilation)
        for (int ich = 0; ich < in_channels; ich++) {
  ...
  for (int och = 0; och < out_channels; och++)
    for (int krow = 0; krow < kernel_dim_dilated; krow++)
      for (int kcol = 0; kcol < kernel_dim_dilated; kcol++)
        for (int kch = 0; kch < in_channels; kch++)
          weights_dilated[och][krow][kcol][kch] = 0;

  idx = 0;
  for (int och = 0; och < out_channels; och++)
    for (int krow = 0; krow < kernel_dim_dilated; krow += kernel_dilation)
      for (int kcol = 0; kcol < kernel_dim_dilated; kcol += kernel_dilation)
        for (int kch = 0; kch < in_channels; kch++) {
          weights_dilated[och][krow][kcol][kch] = *((elem_t *)weights + idx);
          idx++;
        }
          ...
          for (int krow = 0; krow < kernel_dim_dilated; krow++) {
            for (int kcol = 0; kcol < kernel_dim_dilated; kcol++) {
              for (int kch = 0; kch < in_channels; kch++) {
                ...
                elem_t pixel = irow < 0 || irow >= in_row_dim_dilated ||
                                       icol < 0 || icol >= in_col_dim_dilated
                                   ? 0
                                   : dilated[b][irow][icol][kch];

                result += weights_dilated[och][krow][kcol][kch] * pixel;
              }
            }
          }
...
int main() {
...
#ifndef FAST
  conv(BATCH_SIZE, IN_CHANNELS, IN_ROW_DIM, IN_COL_DIM, OUT_CHANNELS,
       KERNEL_DIM, OUT_ROW_DIM, OUT_COL_DIM, STRIDE, INPUT_DILATION,
       KERNEL_DILATION, PADDING, input, weights, bias, output);
#endif
  ...
  tiled_conv_auto(BATCH_SIZE, IN_ROW_DIM, IN_COL_DIM, IN_CHANNELS, OUT_CHANNELS,
                  OUT_ROW_DIM, OUT_COL_DIM, STRIDE, INPUT_DILATION,
                  KERNEL_DILATION, PADDING, KERNEL_DIM, false, false, false,
                  false, false,

                  (elem_t *)input, (elem_t *)weights_mat,
                  NO_BIAS ? NULL : (acc_t *)bias, (elem_t *)output_mat,
...
#else
  bool success = vec_is_equal(&output[0][0][0][0], &output_mat[0][0],
                              sizeof(output) / sizeof(elem_t));
#endif
