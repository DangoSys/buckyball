...
#define IN_STRIDE (IN_CHANNELS + 3)
...
void conv(int batch_size, int in_channels, int in_row_dim, int in_col_dim,
          ...
          int stride, int padding, int in_stride, int out_stride,
...
int main() {
  ...
  conv(BATCH_SIZE, IN_CHANNELS, IN_ROW_DIM, IN_COL_DIM, OUT_CHANNELS,
       KERNEL_DIM, OUT_ROW_DIM, OUT_COL_DIM, STRIDE, PADDING, IN_STRIDE,
       OUT_STRIDE, input, weights, bias, output);
  ...
  flatten_weights(OUT_CHANNELS, KERNEL_DIM, IN_CHANNELS, PATCH_SIZE,
                  WEIGHT_STRIDE, weights, weights_mat);

  printf("Gemmini conv...\n");
  uint64_t start_gemmini = read_cycles();
  tiled_conv_stride_auto(BATCH_SIZE, IN_ROW_DIM, IN_COL_DIM, IN_CHANNELS,
                         OUT_CHANNELS, OUT_ROW_DIM, OUT_COL_DIM, STRIDE, 1, 1,
                         PADDING, KERNEL_DIM, IN_STRIDE, WEIGHT_STRIDE,
                         OUT_STRIDE, false, false, false, false, false,
