...
int full_is_equal(elem_t x[MAT_DIM_I][J_STRIDE],
                  elem_t y[MAT_DIM_I][J_STRIDE]) {
  for (size_t i = 0; i < MAT_DIM_I; ++i)
    for (size_t j = 0; j < MAT_DIM_J; ++j)
      if (x[i][j] != y[i][j])
        return 0;
...
int main() {
  ...
  resadd_cpu(MAT_DIM_I, MAT_DIM_J, J_STRIDE, A_SCALE, B_SCALE, C_SCALE,
             (elem_t *)A, (elem_t *)B, (elem_t *)gold, USE_RELU);
  ...
  tiled_resadd_stride_auto(MAT_DIM_I, MAT_DIM_J, A_SCALE, B_SCALE, C_SCALE,
                           J_STRIDE, (elem_t *)A, (elem_t *)B, (elem_t *)C,
                           USE_RELU, WS);
