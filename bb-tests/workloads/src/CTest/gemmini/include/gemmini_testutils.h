...
static void matmul(elem_t A[DIM][DIM], elem_t B[DIM][DIM], elem_t D[DIM][DIM],
                   full_t C_full[DIM][DIM]) {
...
static void matmul_short(elem_t A[DIM][DIM], elem_t B[DIM][DIM],
                         elem_t D[DIM][DIM], elem_t C[DIM][DIM]) {
  for (size_t r = 0; r < DIM; r++)
    for (size_t c = 0; c < DIM; c++) {
      C[r][c] = D[r][c];
      for (size_t k = 0; k < DIM; k++)
...
static void matmul_full(elem_t A[DIM][DIM], elem_t B[DIM][DIM],
                        full_t D[DIM][DIM], full_t C_full[DIM][DIM]) {
...
static void matmul_A_transposed(elem_t A[DIM][DIM], elem_t B[DIM][DIM],
                                elem_t D[DIM][DIM], full_t C_full[DIM][DIM]) {
  for (size_t r = 0; r < DIM; r++)
    for (size_t c = 0; c < DIM; c++) {
      C_full[r][c] = D[r][c];
      for (size_t k = 0; k < DIM; k++)
...
static void matmul_short_A_transposed(elem_t A[DIM][DIM], elem_t B[DIM][DIM],
                                      elem_t D[DIM][DIM], elem_t C[DIM][DIM]) {
  for (size_t r = 0; r < DIM; r++)
    for (size_t c = 0; c < DIM; c++) {
      C[r][c] = D[r][c];
...
static void matmul_full_A_transposed(elem_t A[DIM][DIM], elem_t B[DIM][DIM],
                                     full_t D[DIM][DIM],
                                     full_t C_full[DIM][DIM]) {
  for (size_t r = 0; r < DIM; r++)
    for (size_t c = 0; c < DIM; c++) {
...
static void matmul_B_transposed(elem_t A[DIM][DIM], elem_t B[DIM][DIM],
                                elem_t D[DIM][DIM], full_t C_full[DIM][DIM]) {
  for (size_t r = 0; r < DIM; r++)
    for (size_t c = 0; c < DIM; c++) {
      C_full[r][c] = D[r][c];
...
static void matmul_short_B_transposed(elem_t A[DIM][DIM], elem_t B[DIM][DIM],
                                      elem_t D[DIM][DIM], elem_t C[DIM][DIM]) {
  for (size_t r = 0; r < DIM; r++)
    for (size_t c = 0; c < DIM; c++) {
      C[r][c] = D[r][c];
...
static void matmul_full_B_transposed(elem_t A[DIM][DIM], elem_t B[DIM][DIM],
                                     full_t D[DIM][DIM],
                                     full_t C_full[DIM][DIM]) {
  for (size_t r = 0; r < DIM; r++)
    for (size_t c = 0; c < DIM; c++) {
...
static void matmul_AB_transposed(elem_t A[DIM][DIM], elem_t B[DIM][DIM],
                                 elem_t D[DIM][DIM], full_t C_full[DIM][DIM]) {
  for (size_t r = 0; r < DIM; r++)
    for (size_t c = 0; c < DIM; c++) {
      C_full[r][c] = D[r][c];
...
static void matmul_short_AB_transposed(elem_t A[DIM][DIM], elem_t B[DIM][DIM],
                                       elem_t D[DIM][DIM], elem_t C[DIM][DIM]) {
  for (size_t r = 0; r < DIM; r++)
    for (size_t c = 0; c < DIM; c++) {
      C[r][c] = D[r][c];
...
static void matmul_full_AB_transposed(elem_t A[DIM][DIM], elem_t B[DIM][DIM],
                                      full_t D[DIM][DIM],
                                      full_t C_full[DIM][DIM]) {
  for (size_t r = 0; r < DIM; r++)
    for (size_t c = 0; c < DIM; c++) {
...
static void matadd(full_t sum[DIM][DIM], full_t m1[DIM][DIM],
                   full_t m2[DIM][DIM]) {
  for (size_t r = 0; r < DIM; r++)
    for (size_t c = 0; c < DIM; c++)
...
static void matshift(full_t full[DIM][DIM], elem_t out[DIM][DIM], int shift) {
  for (size_t r = 0; r < DIM; r++)
    ...
    // Saturate and cast element
      ...
      full_t elem = shifted > elem_t_max
                        ? elem_t_max
...
static void matscale(full_t full[DIM][DIM], elem_t out[DIM][DIM],
                     acc_scale_t scale) {
  for (size_t r = 0; r < DIM; r++)
    for (size_t c = 0; c < DIM; c++) {
      // Bitshift and round element
