...
int main(int argc, char *argv[]) {
  ...
  if (argc == 9) {
    ...
    printf("BATCH_SIZE IN_DIM IN_CHANNELS OUT_CHANNELS KERNEL_DIM PADDING "
           "STRIDE NO_BIAS\n");
    exit(1);
  }
  ...
  elem_t *weights =
      (elem_t *)((elem_t *)input + BATCH_SIZE * IN_DIM * IN_DIM * IN_CHANNELS);
  acc_t *bias = (acc_t *)((elem_t *)weights +
                          OUT_CHANNELS * KERNEL_DIM * KERNEL_DIM * IN_CHANNELS);
  elem_t *output = (elem_t *)((acc_t *)bias + OUT_CHANNELS);

  {
    uint8_t *end = (uint8_t *)((elem_t *)output +
                               BATCH_SIZE * OUT_DIM * OUT_DIM * OUT_CHANNELS);
  ...
  if (map_to_matmul) {
    ...
    tiled_matmul_auto(I, J, K, input, weights, NO_BIAS ? NULL : bias, output, K,
    ...
    tiled_conv_auto(BATCH_SIZE, IN_DIM, IN_DIM, IN_CHANNELS, OUT_CHANNELS,
                    OUT_DIM, OUT_DIM, STRIDE, 1, 1, PADDING, KERNEL_DIM, false,
                    false, false, false, false,

                    (elem_t *)input, (elem_t *)weights,
                    NO_BIAS ? NULL : (acc_t *)bias, (elem_t *)output,
