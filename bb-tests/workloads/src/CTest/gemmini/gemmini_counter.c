...
int main() {
  ...
  // Check value (should be increasing right now as Gemmini executes in the
  // background)
  ...
  // Print first counter value (Syscall takes a lot of time, and we might not
  // capture a snapshot when the instructions are still being executed)
  ...
  printf("Cycle when taking snapshot: %d, Cycle read after operation finished: "
         "%d\n",
  ...
  if (counter_val < snapshot_val + 10) {
    printf("Counter values changed too little after snapshot reset; check if "
           "counter continues properly\n");
    exit(1);
  }
  ...
  printf("RESERVATION_STATION # of load insts after executing %d mvin and "
         "mvout insts: %d\n",
         N - 1, counter_val);
  if (counter_val < 2) {
  ...
  if (counter_val != snapshot_val) {
    printf("Snapshot value doesn't match the raw value read before snapshot "
           "taken\n");
    exit(1);
  }
