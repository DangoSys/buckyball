...
void matscale_big(full_t full[BIG_DIM][BIG_DIM], elem_t out[BIG_DIM][BIG_DIM],
                  acc_scale_t scale) {
...
int main() {
  ...
  for (int block_len = BINIT;
       block_len <= BIG_DIM / DIM && block_len <= MAX_BLOCK_LEN_ACC;
       block_len++) {
...
#define RAND rand()
        ...
        for (size_t i = 0; i < BIG_DIM; i += DIM) {
          for (size_t j = 0; j < BIG_DIM; j += DIM) {
            ...
            if (!already_moved_in) {
              int len = j + block_len * DIM <= BIG_DIM ? block_len
                                                       : (BIG_DIM - j) / DIM;
