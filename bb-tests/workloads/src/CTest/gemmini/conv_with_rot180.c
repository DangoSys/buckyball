...
#define OUT_ROW_DIM                                                            \
  ((IN_ROW_DIM_DILATED + 2 * PADDING - KERNEL_DIM) / STRIDE + 1)
#define OUT_COL_DIM                                                            \
  ((IN_COL_DIM_DILATED + 2 * PADDING - KERNEL_DIM) / STRIDE + 1)
...
void conv(int batch_size, int in_channels, int in_row_dim, int in_col_dim,
  ...
  static elem_t weights_rot180[OUT_CHANNELS][KERNEL_DIM][KERNEL_DIM]
                              [IN_CHANNELS];
          ...
          for (int krow = 0; krow < kernel_dim; krow++) {
            for (int kcol = 0; kcol < kernel_dim; kcol++) {
              for (int kch = 0; kch < in_channels; kch++) {
                ...
                elem_t w = wrot180 ? weights_rot180[och][krow][kcol][kch]
...
int main() {
  ...
  conv(BATCH_SIZE, IN_CHANNELS, IN_ROW_DIM, IN_COL_DIM, OUT_CHANNELS,
       KERNEL_DIM, OUT_ROW_DIM, OUT_COL_DIM, STRIDE, INPUT_DILATION, PADDING,
       WROT180, input, weights, bias, output);
#endif
  ...
  tiled_conv_auto(BATCH_SIZE, IN_ROW_DIM, IN_COL_DIM, IN_CHANNELS, OUT_CHANNELS,
                  OUT_ROW_DIM, OUT_COL_DIM, STRIDE, INPUT_DILATION, 1, PADDING,
                  KERNEL_DIM, WROT180, false, false, false, false,
