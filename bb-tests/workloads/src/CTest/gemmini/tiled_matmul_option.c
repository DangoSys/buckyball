...
void full_matmul_At(elem_t A[MAT_DIM_K][MAT_DIM_I],
...
void full_matmul_Bt(elem_t A[MAT_DIM_I][MAT_DIM_K],
                    elem_t B[MAT_DIM_J][MAT_DIM_K],
                    acc_t D[MAT_DIM_I][MAT_DIM_J],
                    full_t C_full[MAT_DIM_I][MAT_DIM_J], bool repeating_bias) {
  for (size_t r = 0; r < MAT_DIM_I; r++)
    for (size_t c = 0; c < MAT_DIM_J; c++) {
...
void full_matmul_At_Bt(elem_t A[MAT_DIM_K][MAT_DIM_I],
                       elem_t B[MAT_DIM_J][MAT_DIM_K],
                       acc_t D[MAT_DIM_I][MAT_DIM_J],
                       full_t C_full[MAT_DIM_I][MAT_DIM_J],
                       bool repeating_bias) {
  for (size_t r = 0; r < MAT_DIM_I; r++)
    for (size_t c = 0; c < MAT_DIM_J; c++) {
      C_full[r][c] = D[repeating_bias ? 0 : r][c];
...
void full_matscale(full_t full[MAT_DIM_I][MAT_DIM_J],
                   elem_t out[MAT_DIM_I][MAT_DIM_J], acc_scale_t scale) {
  for (size_t r = 0; r < MAT_DIM_I; r++)
    for (size_t c = 0; c < MAT_DIM_J; c++) {
...
void full_matrelu(elem_t in[MAT_DIM_I][MAT_DIM_J],
...
int full_is_equal(elem_t x[MAT_DIM_I][MAT_DIM_J],
                  elem_t y[MAT_DIM_I][MAT_DIM_J]) {
  for (size_t i = 0; i < MAT_DIM_I; ++i)
    for (size_t j = 0; j < MAT_DIM_J; ++j)
      if (x[i][j] != y[i][j])
        return 0;
...
#pragma GCC diagnostic ignored "-Wincompatible-pointer-types"
                ...
                if (!a_transpose && !b_transpose) {
                  ...
                  full_matmul_At(full_A, full_B, full_D, gold_full,
                                 repeating_bias);
                } else if (!a_transpose && b_transpose) {
                  full_matmul_Bt(full_A, full_B, full_D, gold_full,
                                 repeating_bias);
                } else if (a_transpose && b_transpose) {
                  full_matmul_At_Bt(full_A, full_B, full_D, gold_full,
                                    repeating_bias);
                }
...
#pragma GCC diagnostic pop
                ...
                tiled_matmul_auto(
                    MAT_DIM_I, MAT_DIM_J, MAT_DIM_K, (elem_t *)full_A,
                    (elem_t *)full_B, no_bias ? NULL : &full_D[0][0],
                    (elem_t *)full_C, stride_A, stride_B, MAT_DIM_J, MAT_DIM_J,
                    MVIN_SCALE_IDENTITY, MVIN_SCALE_IDENTITY,
                    MVIN_SCALE_IDENTITY, activation, scale, 0, repeating_bias,
                    a_transpose, b_transpose, false, false, 0, option);
