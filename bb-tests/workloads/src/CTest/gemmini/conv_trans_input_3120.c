...
#define TRANS_WEIGHT_0132 false
#define TRANS_INPUT_3120 true
...
int main() {
  ...
  static elem_t input[IN_ROW_DIM][IN_COL_DIM][IN_CHANNELS][BATCH_SIZE];
  ...
  tiled_conv_auto(BATCH_SIZE, IN_ROW_DIM, IN_COL_DIM, IN_CHANNELS, OUT_CHANNELS,
                  ...
                  false, TRANS_OUTPUT_1203, TRANS_INPUT_3120, TRANS_WEIGHT_1203,
                  TRANS_WEIGHT_0132,
    ...
    for (int batch = 0; batch < BATCH_SIZE; batch++) {
      ...
      for (int irow = 0; irow < IN_ROW_DIM; irow++) {
        ...
        for (int icol = 0; icol < IN_COL_DIM; icol++) {
          printf("[");
          for (int ich = 0; ich < IN_CHANNELS; ich++) {
            printf("%d,", input[irow][icol][ich][batch]);
          }
          printf("\b],");
        }
        printf("\b],\n");
