...
void pool(int batch_size, int channels, int in_row_dim, int in_col_dim,
          int out_row_dim, int out_col_dim, int window_dim, int stride,
          int padding,
...
int main() {
  ...
  conv(BATCH_SIZE, IN_CHANNELS, IN_ROW_DIM, IN_COL_DIM, OUT_CHANNELS,
       KERNEL_DIM, OUT_ROW_DIM, OUT_COL_DIM, STRIDE, PADDING, input, weights,
       bias, output);
  ...
  pool(BATCH_SIZE, OUT_CHANNELS, OUT_ROW_DIM, OUT_COL_DIM, POOL_OUT_ROW_DIM,
       POOL_OUT_COL_DIM, POOL_SIZE, POOL_STRIDE, POOL_PADDING, output,
       pool_output);
  ...
  static elem_t pool_output_mat[BATCH_SIZE * POOL_OUT_ROW_DIM *
                                POOL_OUT_COL_DIM][OUT_CHANNELS];
  ...
  tiled_conv_auto(BATCH_SIZE, IN_ROW_DIM, IN_COL_DIM, IN_CHANNELS, OUT_CHANNELS,
                  ...
                  NO_BIAS ? NULL : (acc_t *)bias, (elem_t *)pool_output_mat,

                  NO_ACTIVATION, ACC_SCALE_IDENTITY, POOL_SIZE,
...
#ifdef FAST
  bool success = true;
  for (int orow = 0; orow < BATCH_SIZE * POOL_OUT_ROW_DIM * POOL_OUT_COL_DIM;
       orow++) {
...
#else
  bool success = vec_is_equal(&pool_output[0][0][0][0], &pool_output_mat[0][0],
                              sizeof(pool_output) / sizeof(elem_t));
#endif
