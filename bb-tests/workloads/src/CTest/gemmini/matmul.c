...
// The main point of this test is just to check whether we can switch between
// output- and weight-stationary dataflows
...
void test_os(bool A_transpose, bool B_transpose) {
  ...
  void (*matmul_ptr)(elem_t[DIM][DIM], elem_t[DIM][DIM], elem_t[DIM][DIM],
                     full_t[DIM][DIM]);
  void (*matmul_full_ptr)(elem_t[DIM][DIM], elem_t[DIM][DIM], full_t[DIM][DIM],
                          full_t[DIM][DIM]);
  ...
  for (int activation = AINIT; activation <= RELU; ++activation) {
    for (int shift = SINIT; shift <= 4; shift += 4) {
      ...
      // printf("Setting mode\n");
      gemmini_extended_config_ex(OUTPUT_STATIONARY, activation, shift, 1,
      ...
      */
      ...
      for (int n = 0; n < N * N * N; ++n)
        if (!no_output[n] && !is_equal(C[n], gold[n])) {
          printf("activation: %d, shift: %d, n: %d, A_transpose: %d, "
                 "B_transpose: %d\n",
...
void test_ws(bool A_transpose, bool B_transpose) {
  ...
  for (int activation = AINIT; activation <= RELU; ++activation) {
    for (int scale = SINIT; scale <= 4; scale += 4) {
      ...
      // ...taking into account whether we preload new weights or re-use the old
      // ones
      static int preload[N * N * N] = {1};
      for (int i = 1; i < N * N * N; ++i)
        preload[i] = RAND % 2;
      ...
      for (size_t g = 0; g < N * N * N; ++g) {
        ...
        // We need to find the last B value in case we aren't preloading new
        // weights
      ...
      // printf("Setting mode\n");
      gemmini_extended_config_ex(WEIGHT_STATIONARY, 0, 0, 1, A_transpose,
                                 B_transpose);
      ...
      for (int n = 0; n < N * N * N; ++n)
        if (!no_output[n] && !is_equal(C[n], gold[n])) {
          printf("activation: %d, scale: %d, n: %d, A_transpose: %d, "
                 "B_transpose: %d\n",
...
int main() {
  ...
  for (int A_transpose = 0; A_transpose < 2; A_transpose++) {
    for (int B_transpose = 0; B_transpose < 2; B_transpose++) {
      for (int dataflow = OUTPUT_STATIONARY; dataflow <= WEIGHT_STATIONARY;
           dataflow++) {
        printf("A_transpose: %d, B_transpose: %d, dataflow: %d\n", A_transpose,
               B_transpose, dataflow);
