...
int test_random3() {
  init_u8_random_matrix(input_matrix_a, DIM, 48, 333);
  init_u8_random_matrix(input_matrix_b, 48, DIM, 444);
  return run_test("Random matrices 3", input_matrix_a, input_matrix_b, 48);
}
...
int main() {
  ...
  multicore(MULTICORE);
#endif
  int passed = test_random3();
  if (passed) {
    printf("vecunit_matmul_16xn_random3 test PASSED\n");
    return 0;
  } else {
    printf("vecunit_matmul_16xn_random3 test FAILED\n");
    return 1;
  }
#ifdef MULTICORE
  exit(0);
