...
int test_random1() {
  init_bbfp_random_matrix(input_matrix_a, DIM, DIM, 456);
  init_bbfp_random_matrix(input_matrix_b, DIM, DIM, 789);
...
int main() {
  ...
  if (passed) {
    printf("bbfp_matmul_random1 test PASSED\n");
  } else {
    printf("bbfp_matmul_random1 test FAILED\n");
  }
#ifdef MULTICORE
  exit(0);
#endif
